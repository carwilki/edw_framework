#Code converted on 2023-06-12 20:31:03
import os
...
# read_infa_paramfile('', 'm_WM_Asn') ProcessingUtils

# COMMAND ----------
# Processing node SQ_Shortcut_to_WM_ASN_PRE, type SOURCE 
# COLUMN COUNT: 142

SQ_Shortcut_to_WM_ASN_PRE = spark.read.jdbc(os.environ.get('SCDS_CONNECT_STRING'), f"""SELECT
WM_ASN_PRE.DC_NBR,
WM_ASN_PRE.ASN_ID,
...
FROM WM_ASN_PRE""", 
properties={
'user': os.environ.get('SCDS_LOGIN'),
'password': os.environ.get('SCDS_PASSWORD'),
...
SQ_Shortcut_to_WM_ASN = spark.read.jdbc(os.environ.get('_CONNECT_STRING'), f"""SELECT
WM_ASN.LOCATION_ID,
WM_ASN.WM_ASN_ID,
...
Shortcut_to_WM_ASN1 = UPD_INS_UPD.selectExpr( \
	...
	"CAST(TC_ASN_ID AS VARCHAR) as WM_TC_ASN_ID", \
	"CAST(TC_ASN_ID_U AS VARCHAR) as WM_TC_ASN_ID_UPPER_CASE", \
	...
	"CAST(ASN_SHPMT_TYPE AS VARCHAR) as WM_ASN_SHPMT_TYPE", \
	"CAST(ASN_STATUS AS BIGINT) as WM_ASN_STATUS", \
	"CAST(APPOINTMENT_ID AS VARCHAR) as WM_APPOINTMENT_ID", \
	...
	"CAST(TC_SHIPMENT_ID AS VARCHAR) as WM_TC_SHIPMENT_ID", \
	"CAST(INVOICE_NUMBER AS VARCHAR) as WM_INVOICE_NBR", \
	"CAST(INVOICE_DATE AS TIMESTAMP) as WM_INVOICE_TSTMP", \
	"CAST(DC_ORD_NBR AS VARCHAR) as WM_DC_ORD_NBR", \
	"CAST(WORK_ORD_NBR AS VARCHAR) as WM_WORK_ORD_NBR", \
	"CAST(ORIGINAL_ASN_ID AS BIGINT) as WM_ORIGINAL_ASN_ID", \
	"CAST(ASN_ORGN_TYPE AS VARCHAR) as WM_ASN_ORGN_TYPE", \
	"CAST(ORIGINAL_ORDER_NUMBER AS VARCHAR) as WM_ORIGINAL_ORDER_NBR", \
	"CAST(ORIGIN_FACILITY_ALIAS_ID AS VARCHAR) as WM_ORIGIN_FACILITY_ALIAS_ID", \
	"CAST(ORIGIN_FACILITY_ID AS BIGINT) as WM_ORIGIN_FACILITY_ID", \
	"CAST(DESTINATION_FACILITY_ALIAS_ID AS VARCHAR) as WM_DESTINATION_FACILITY_ALIAS_ID", \
	"CAST(DESTINATION_FACILITY_ID AS BIGINT) as WM_DESTINATION_FACILITY_ID", \
	"CAST(RETURN_REFERENCE_NUMBER AS VARCHAR) as WM_RETURN_REFERENCE_NBR", \
	...
	"CAST(FLOW_THROUGH_ALLOCATION_METHOD AS VARCHAR) as WM_FLOW_THROUGH_ALLOCATION_METHOD", \
	...
	"CAST(BILL_OF_LADING_NUMBER AS VARCHAR) as WM_BILL_OF_LADING_NBR", \
	"CAST(MANIF_NBR AS VARCHAR) as WM_MANIF_NBR", \
	"CAST(MANIF_TYPE AS VARCHAR) as WM_MANIF_TYPE", \
	"CAST(PRO_NUMBER AS VARCHAR) as WM_PRO_NBR", \
	"CAST(EQUIPMENT_CODE_ID AS BIGINT) as WM_EQUIPMENT_CD_ID", \
	"CAST(EQUIPMENT_CODE AS VARCHAR) as WM_EQUIPMENT_CD", \
	"CAST(EQUIPMENT_TYPE AS VARCHAR) as WM_EQUIPMENT_TYPE", \
	"CAST(TRACTOR_NUMBER AS VARCHAR) as WM_TRACTOR_NBR", \
	"CAST(TRACTOR_CARRIER_CODE_ID AS BIGINT) as WM_TRACTOR_CARRIER_CD_ID", \
	"CAST(TRACTOR_CARRIER_CODE AS VARCHAR) as WM_TRACTOR_CARRIER_CD", \
	"CAST(REGION_ID AS BIGINT) as WM_REGION_ID", \
	"CAST(INBOUND_REGION_ID AS BIGINT) as WM_INBOUND_REGION_ID", \
	"CAST(INBOUND_REGION_NAME AS VARCHAR) as WM_INBOUND_REGION_NAME", \
	...
	"CAST(OUTBOUND_REGION_NAME AS VARCHAR) as WM_OUTBOUND_REGION_NAME", \
	"CAST(ASSIGNED_CARRIER_CODE_ID AS BIGINT) as WM_ASSIGNED_CARRIER_CD_ID", \
	"CAST(ASSIGNED_CARRIER_CODE AS VARCHAR) as WM_ASSIGNED_CARRIER_CD", \
	"CAST(CUT_NBR AS VARCHAR) as WM_CUT_NBR", \
	"CAST(MFG_PLNT AS VARCHAR) as WM_MFG_PLNT", \
	"CAST(DOCK_DOOR_ID AS BIGINT) as WM_DOCK_DOOR_ID", \
	"CAST(MODE_ID AS BIGINT) as WM_MODE_ID", \
	"CAST(TRAILER_NUMBER AS VARCHAR) as WM_TRAILER_NBR", \
	"CAST(TRAILER_CLOSED AS BIGINT) as TRAILER_CLOSED_FLAG", \
	"CAST(DESTINATION_TYPE AS VARCHAR) as WM_DESTINATION_TYPE", \
	"CAST(VARIANCE_TYPE AS BIGINT) as WM_VARIANCE_TYPE", \
	"CAST(CONTRAC_LOCN AS VARCHAR) as WM_CONTRAC_LOCN", \
	"CAST(MHE_SENT AS VARCHAR) as WM_MHE_SENT", \
	"CAST(BUYER_CODE AS VARCHAR) as WM_BUYER_CD", \
	"CAST(REP_NAME AS VARCHAR) as WM_REP_NAME", \
	...
	"CAST(INITIATE_FLAG AS VARCHAR) as INITIATE_IND", \
	"CAST(ALLOCATION_COMPLETED AS BIGINT) as ALLOCATION_COMPLETED_FLAG", \
	"CAST(PALLET_FOOTPRINT AS VARCHAR) as WM_PALLET_FOOTPRINT", \
	...
	"CAST(SHIPPING_COST_CURRENCY_CODE AS VARCHAR) as SHIPPING_COST_CURRENCY_CD", \
	"CAST(CONTACT_NUMBER AS VARCHAR) as WM_CONTACT_NBR", \
	"CAST(CONTACT_ADDRESS_1 AS VARCHAR) as CONTACT_ADDR_1", \
	"CAST(CONTACT_ADDRESS_2 AS VARCHAR) as CONTACT_ADDR_2", \
	"CAST(CONTACT_ADDRESS_3 AS VARCHAR) as CONTACT_ADDR_3", \
	"CAST(CONTACT_CITY AS VARCHAR) as CONTACT_CITY", \
	"CAST(CONTACT_STATE_PROV AS VARCHAR) as CONTACT_STATE_PROV", \
	"CAST(CONTACT_ZIP AS VARCHAR) as CONTACT_ZIP", \
	"CAST(CONTACT_COUNTY AS VARCHAR) as CONTACT_COUNTY", \
	"CAST(CONTACT_COUNTRY_CODE AS VARCHAR) as CONTACT_COUNTRY_CD", \
	...
	"CAST(REF_FIELD_3 AS VARCHAR) as REF_FIELD_3", \
	...
	"CAST(MISC_INSTR_CODE_1 AS VARCHAR) as MISC_INSTR_CD_1", \
	"CAST(MISC_INSTR_CODE_2 AS VARCHAR) as MISC_INSTR_CD_2", \
...
Shortcut_to_WM_ASN1.write.saveAsTable('WM_ASN', mode = 'append')