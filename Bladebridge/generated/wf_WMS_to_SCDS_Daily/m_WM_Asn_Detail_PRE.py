#Code converted on 2023-06-12 20:30:46
import os
...
# read_infa_paramfile('', 'm_WM_Asn_Detail_PRE') ProcessingUtils
...
SQ_Shortcut_to_ASN_DETAIL = spark.read.jdbc(os.environ.get('OR_DC_WMS_CONNECT_STRING'), f"""SELECT
...
FROM ASN_DETAIL
WHERE $$Initial_Load (date_trunc('DD', CREATED_DTTM) >= date_trunc('DD', to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1) OR (date_trunc('DD', LAST_UPDATED_DTTM) >=  date_trunc('DD', to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1) AND
1=1""", 
properties={
...
#<CHANGE_NEEDED>
#JDBC connection should be configured from the config call
...
Shortcut_to_WM_ASN_DETAIL_PRE = EXPTRANS.selectExpr( \
	...
	"CAST(TC_PURCHASE_ORDERS_ID AS VARCHAR) as TC_PURCHASE_ORDERS_ID", \
	...
	"CAST(SKU_ATTR_1 AS VARCHAR) as SKU_ATTR_1", \
	"CAST(SKU_ATTR_2 AS VARCHAR) as SKU_ATTR_2", \
	"CAST(SKU_ATTR_3 AS VARCHAR) as SKU_ATTR_3", \
	"CAST(SKU_ATTR_4 AS VARCHAR) as SKU_ATTR_4", \
	"CAST(SKU_ATTR_5 AS VARCHAR) as SKU_ATTR_5", \
	"CAST(BUSINESS_PARTNER_ID AS VARCHAR) as BUSINESS_PARTNER_ID", \
	"CAST(PACKAGE_TYPE_ID AS BIGINT) as PACKAGE_TYPE_ID", \
	"CAST(PACKAGE_TYPE_DESC AS VARCHAR) as PACKAGE_TYPE_DESC", \
	"CAST(PACKAGE_TYPE_INSTANCE AS VARCHAR) as PACKAGE_TYPE_INSTANCE", \
	"CAST(EPC_TRACKING_RFID_VALUE AS VARCHAR) as EPC_TRACKING_RFID_VALUE", \
	"CAST(ORDER_TYPE_DESC AS VARCHAR) as ORDER_TYPE_DESC", \
	...
	"CAST(EPC_REQ_ON_ALL_CASES AS VARCHAR) as EPC_REQ_ON_ALL_CASES", \
	...
	"CAST(INVN_TYPE AS VARCHAR) as INVN_TYPE", \
	"CAST(PROD_STAT AS VARCHAR) as PROD_STAT", \
	"CAST(BATCH_NBR AS VARCHAR) as BATCH_NBR", \
	"CAST(CNTRY_OF_ORGN AS VARCHAR) as CNTRY_OF_ORGN", \
	...
	"CAST(PROC_IMMD_NEEDS AS VARCHAR) as PROC_IMMD_NEEDS", \
	"CAST(QUALITY_CHECK_HOLD_UPON_RCPT AS VARCHAR) as QUALITY_CHECK_HOLD_UPON_RCPT", \
	"CAST(REFERENCE_ORDER_NBR AS VARCHAR) as REFERENCE_ORDER_NBR", \
	...
	"CAST(PUTWY_TYPE AS VARCHAR) as PUTWY_TYPE", \
	"CAST(RETAIL_PRICE AS BIGINT) as RETAIL_PRICE", \
	"CAST(PRICE_TIX_AVAIL AS VARCHAR) as PRICE_TIX_AVAIL", \
	...
	"CAST(TC_PO_LINE_ID AS VARCHAR) as TC_PO_LINE_ID", \
	"CAST(INVENTORY_SEGMENT_ID AS BIGINT) as INVENTORY_SEGMENT_ID", \
	"CAST(PPACK_GRP_CODE AS VARCHAR) as PPACK_GRP_CODE", \
	"CAST(CUT_NBR AS VARCHAR) as CUT_NBR", \
	...
	"CAST(TC_ORDER_ID AS VARCHAR) as TC_ORDER_ID", \
	"CAST(ORDER_ID AS BIGINT) as ORDER_ID", \
	"CAST(TC_ORDER_LINE_ID AS VARCHAR) as TC_ORDER_LINE_ID", \
	"CAST(ORDER_LINE_ITEM_ID AS BIGINT) as ORDER_LINE_ITEM_ID", \
	"CAST(SEQ_NBR AS VARCHAR) as SEQ_NBR", \
	"CAST(EXP_RECEIVE_CONDITION_CODE AS VARCHAR) as EXP_RECEIVE_CONDITION_CODE", \
	...
	"CAST(REF_FIELD_2 AS VARCHAR) as REF_FIELD_3", \
	"CAST(REF_FIELD_3 AS VARCHAR) as REF_FIELD_2", \
	...
	"CAST(DISPOSITION_TYPE AS VARCHAR) as DISPOSITION_TYPE", \
	"CAST(PRE_RECEIPT_STATUS AS VARCHAR) as PRE_RECEIPT_STATUS", \
	"CAST(INV_DISPOSITION AS VARCHAR) as INV_DISPOSITION", \
	"CAST(EXT_PLAN_ID AS VARCHAR) as EXT_PLAN_ID", \
...
Shortcut_to_WM_ASN_DETAIL_PRE.write.saveAsTable(f'{table}', mode = 'append')