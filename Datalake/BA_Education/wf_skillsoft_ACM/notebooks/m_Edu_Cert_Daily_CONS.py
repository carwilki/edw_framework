...
SELECT
  ...
  CAST(regexp_replace(ACM.SCORE, r'\.','') AS BIGINT) AS LAST_TEST_SCORE_NBR,
  CAST(CASE WHEN ACM.STATUS IN ('C', 'EQ') THEN 1 ELSE 0 END AS TINYINT) AS LAST_TEST_PASSED_FLAG,
  ACM.DATE_COMPLETED AS COMPLIANT_START_DT,
  ACM.COMPLIANT_EXPIRATION_DT,
  CAST(CASE WHEN ACM.STATUS IN ('OD') THEN 0 ELSE 1 END AS TINYINT) AS CURR_COMPLIANCE_FLAG,
  CAST(CASE WHEN ACM.STATUS IN ('OD') THEN 1 ELSE 0 END AS TINYINT) AS CURR_MISSING_FLAG,
  CAST(ACM.CURR_PERIOD_ATTEMPTS_NBR AS SMALLINT),
