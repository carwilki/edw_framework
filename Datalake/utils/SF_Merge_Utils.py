class SnowflakeWriter:
    ...
    def get_clause(self, column_list, clause_type):
      ...
      for k in column_list:
        if clause == '':
          if clause_type == "merge_key" or (clause_type == "update" and k not in self.update_excl_columns):
            clause="base."+k+"=pre."+k
    ...
    def create_upsert_query(self, cols):
      if self.primary_keys is None and not self.primary_keys:
          raise Exception(f"primary_keys cannot be null for write_mode = merge, create SnowflakeWriter with primary_keys")
