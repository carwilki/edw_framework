...
prev_run_dt = spark.sql(f"""select max(prev_run_date) from {schemaName}.log_run_details where table_name='{refine_table_name}' and lower(status)= 'completed'""").collect()[0][0]
