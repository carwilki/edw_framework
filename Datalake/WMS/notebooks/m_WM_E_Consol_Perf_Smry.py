...
EXP_EVAL_VALUES = FIL_NO_CHANGE_REC.select( \
	...
	col('COMP_ASSIGNMENT_ID'), \
	col('REFLECTIVE_CODE'), \
	(when((col('in_LOAD_TSTMP').isNull()) ,(date_format(current_timestamp(),'yyyy-mm-dd hh:mm:ss'))).otherwise(col('in_LOAD_TSTMP'))).alias('LOAD_TSTMP'), \
	(date_format(current_timestamp(),'yyyy-mm-dd hh:mm:ss')).alias('UPDATE_TSTMP'), \
