...
SQ_Shortcut_to_WM_UCL_USER_PRE_query = (
    """SELECT
...
EXP_INT_CONVERSION = SQ_Shortcut_to_WM_UCL_USER_PRE.select(
    ...
    SQ_Shortcut_to_WM_UCL_USER_PRE.CREATED_SOURCE_TYPE_ID.alias(
    ...
    SQ_Shortcut_to_WM_UCL_USER_PRE.LAST_UPDATED_SOURCE_TYPE_ID.alias(
    ...
    SQ_Shortcut_to_WM_UCL_USER_PRE.COMM_METHOD_ID_DURING_BH_1.alias(
        "COMM_METHOD_ID_DURING_BH_1"
    ),
    SQ_Shortcut_to_WM_UCL_USER_PRE.COMM_METHOD_ID_DURING_BH_2.alias(
        "COMM_METHOD_ID_DURING_BH_2"
    ),
    SQ_Shortcut_to_WM_UCL_USER_PRE.COMM_METHOD_ID_AFTER_BH_1.alias(
        "COMM_METHOD_ID_AFTER_BH_1"
    ),
    SQ_Shortcut_to_WM_UCL_USER_PRE.COMM_METHOD_ID_AFTER_BH_2.alias(
        "COMM_METHOD_ID_AFTER_BH_2"
    ),
    SQ_Shortcut_to_WM_UCL_USER_PRE.COMMON_NAME.alias("COMMON_NAME"),
    SQ_Shortcut_to_WM_UCL_USER_PRE.LAST_PASSWORD_CHANGE_DTTM.alias(
    ...
    SQ_Shortcut_to_WM_UCL_USER_PRE.DEFAULT_BUSINESS_UNIT_ID.alias(
        "DEFAULT_BUSINESS_UNIT_ID"
    ),
    SQ_Shortcut_to_WM_UCL_USER_PRE.DEFAULT_WHSE_REGION_ID.alias(
    ...
    SQ_Shortcut_to_WM_UCL_USER_PRE.NUMBER_OF_INVALID_LOGINS.alias(
    ...
    SQ_Shortcut_to_WM_UCL_USER_PRE.PASSWORD_RESET_DATE_TIME.alias(
        "PASSWORD_RESET_DATE_TIME"
    ),
    SQ_Shortcut_to_WM_UCL_USER_PRE.PASSWORD_TOKEN.alias("PASSWORD_TOKEN"),
    SQ_Shortcut_to_WM_UCL_USER_PRE.ISPASSWORDMANAGEDINTERNALLY.alias(
    ...
    SQ_Shortcut_to_WM_UCL_USER_PRE.SECURITY_POLICY_GROUP_ID.alias(
...
JNR_SITE_PROFILE = SQ_Shortcut_to_SITE_PROFILE.join(
    EXP_INT_CONVERSION,
    [SQ_Shortcut_to_SITE_PROFILE.STORE_NBR == EXP_INT_CONVERSION.o_DC_NBR],
    "inner",
).select(
...
JNR_WM_UCL_USER = SQ_Shortcut_to_WM_UCL_USER.join(
    JNR_SITE_PROFILE,
    [
        SQ_Shortcut_to_WM_UCL_USER.LOCATION_ID == JNR_SITE_PROFILE.LOCATION_ID1,
        SQ_Shortcut_to_WM_UCL_USER.USER_NAME == JNR_SITE_PROFILE.USER_NAME,
    ],
    "right_outer",
).select(
...
FIL_UNCHANGED_RECORDS = (
    JNR_WM_UCL_USER.select(
        ...
        JNR_WM_UCL_USER.LAST_UPDATED_SOURCE_TYPE_ID.alias(
            "LAST_UPDATED_SOURCE_TYPE_ID"
        ),
        ...
        JNR_WM_UCL_USER.ISPASSWORDMANAGEDINTERNALLY.alias(
            "ISPASSWORDMANAGEDINTERNALLY"
        ),
    ...
    .filter(
        """(i_USER_NAME1 is null)
        OR ( NOT (i_USER_NAME1 is null)
        AND ( ((case when CREATED_DTTM is null then TO_DATE('01/01/1900','M/d/y') else CREATED_DTTM end )
         != (case when i_WM_CREATED_TSTMP is null then TO_DATE('01/01/1900','M/d/y') else i_WM_CREATED_TSTMP end))
        or ((case when LAST_UPDATED_DTTM is null then TO_DATE('01/01/1900','M/d/y')
        else LAST_UPDATED_DTTM end )!= (case when i_WM_LAST_UPDATED_TSTMP is null
        then TO_DATE('01/01/1900','M/d/y') else i_WM_LAST_UPDATED_TSTMP end))))"""
    )
    .withColumn("sys_row_id", monotonically_increasing_id())
)
...
EXP_UPD_VALIDATOR = FIL_UNCHANGED_RECORDS.select(
    ...
    FIL_UNCHANGED_RECORDS.LAST_UPDATED_SOURCE_TYPE_ID.alias(
        "LAST_UPDATED_SOURCE_TYPE_ID"
    ),
    ...
    FIL_UNCHANGED_RECORDS.COMM_METHOD_ID_DURING_BH_1.alias(
        "COMM_METHOD_ID_DURING_BH_1"
    ),
    FIL_UNCHANGED_RECORDS.COMM_METHOD_ID_DURING_BH_2.alias(
        "COMM_METHOD_ID_DURING_BH_2"
    ),
    ...
    FIL_UNCHANGED_RECORDS.ISPASSWORDMANAGEDINTERNALLY.alias(
        "ISPASSWORDMANAGEDINTERNALLY"
    ),
    ...
    FIL_UNCHANGED_RECORDS.i_LOCATION_ID.alias("i_LOCATION_ID1"),
).select(
    ...
    (
        when((col("i_LOAD_TSTMP").isNull()), (current_timestamp())).otherwise(
            col("i_LOAD_TSTMP")
        )
    ).alias("LOAD_TSTMP"),
    (
        when(
            (((col("i_USER_NAME1").isNull()) & (col("i_LOCATION_ID1").isNull()))),
            (lit(1)),
        ).otherwise(lit(2))
...
Shortcut_to_WM_UCL_USER = UPD_INS_UPD.select(
    ...
    UPD_INS_UPD.SECURITY_POLICY_GROUP_ID.cast(DecimalType(10, 0)).alias(
        "WM_SECURITY_POLICY_GROUP_ID"
    ),
    UPD_INS_UPD.DEFAULT_BUSINESS_UNIT_ID.cast(DecimalType(9, 0)).alias(
        "DEFAULT_WM_BUSINESS_UNIT_ID"
    ),
    UPD_INS_UPD.DEFAULT_WHSE_REGION_ID.cast(DecimalType(9, 0)).alias(
        "DEFAULT_WM_WHSE_REGION_ID"
    ),
    UPD_INS_UPD.CHANNEL_ID.cast(DecimalType(18, 0)).alias("WM_CHANNEL_ID"),
    UPD_INS_UPD.COMM_METHOD_ID_DURING_BH_1.cast(DecimalType(4, 0)).alias(
        "WM_COMM_METHOD_ID_DURING_BH_1"
    ),
    UPD_INS_UPD.COMM_METHOD_ID_DURING_BH_2.cast(DecimalType(4, 0)).alias(
        "WM_COMM_METHOD_ID_DURING_BH_2"
    ),
    UPD_INS_UPD.COMM_METHOD_ID_AFTER_BH_1.cast(DecimalType(4, 0)).alias(
        "WM_COMM_METHOD_ID_AFTER_BH_1"
    ),
    UPD_INS_UPD.COMM_METHOD_ID_AFTER_BH_2.cast(DecimalType(4, 0)).alias(
        "WM_COMM_METHOD_ID_AFTER_BH_2"
    ),
    UPD_INS_UPD.ISPASSWORDMANAGEDINTERNALLY.cast(DecimalType(1, 0)).alias(
    ...
    UPD_INS_UPD.NUMBER_OF_INVALID_LOGINS1.cast(DecimalType(4, 0)).alias(
        "NUMBER_OF_INVALID_LOGINS"
    ),
    UPD_INS_UPD.PASSWORD_RESET_DATE_TIME.cast(TimestampType()).alias(
        "PASSWORD_RESET_TSTMP"
    ),
    UPD_INS_UPD.LAST_PASSWORD_CHANGE_DTTM.cast(TimestampType()).alias(
        "LAST_PASSWORD_CHANGE_TSTMP"
    ),
    UPD_INS_UPD.HIBERNATE_VERSION1.cast(DecimalType(10, 0)).alias(
        "WM_HIBERNATE_VERSION"
    ),
    UPD_INS_UPD.CREATED_SOURCE_TYPE_ID.cast(DecimalType(4, 0)).alias(
    ...
    UPD_INS_UPD.LAST_UPDATED_SOURCE_TYPE_ID.cast(DecimalType(4, 0)).alias(
...
try:
    ...
    logPrevRunDt(
        "WM_UCL_USER", "WM_UCL_USER", "Completed", "N/A", f"{raw}.log_run_details"
    )
except Exception as e:
    logPrevRunDt(
        "WM_UCL_USER", "WM_UCL_USER", "Failed", str(e), f"{raw}.log_run_details"
    )
    raise e
