...
def m_WM_Pick_Locn_Dtl_PRE(dcnbr, env):
    ...
    refine_table_name = "WM_PICK_LOCN_DTL"
    ...
    query = f"""SELECT
                ...
                FROM PICK_LOCN_DTL
                WHERE {Initial_Load} (trunc(PICK_LOCN_DTL.CREATE_DATE_TIME) >= trunc(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (trunc(PICK_LOCN_DTL.MOD_DATE_TIME) >= trunc(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (trunc(PICK_LOCN_DTL.CREATED_DTTM) >= trunc(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (trunc(PICK_LOCN_DTL.LAST_UPDATED_DTTM) >= trunc(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14)"""

    SQ_Shortcut_to_PICK_LOCN_DTL = gu.jdbcOracleConnection(query, username, password, connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    ...
    gu.overwriteDeltaPartition(Shortcut_to_WM_PICK_LOCN_DTL_PRE, "DC_NBR", dcnbr, target_table_name)
