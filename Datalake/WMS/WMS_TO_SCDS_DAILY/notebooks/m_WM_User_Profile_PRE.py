...
def m_WM_User_Profile_PRE(dcnbr, env):
    ...
    refine_table_name = "WM_USER_PROFILE"
    ...
    query = f"""SELECT
                ...
                FROM USER_PROFILE
                WHERE {Initial_Load} (TRUNC(USER_PROFILE.CREATE_DATE_TIME)>= TRUNC(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 1) OR (TRUNC(USER_PROFILE.MOD_DATE_TIME)>= TRUNC(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 1) OR (TRUNC(USER_PROFILE.LAST_UPDATED_DTTM)>= TRUNC(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 1) OR (TRUNC(USER_PROFILE.CREATED_DTTM)>= TRUNC(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 1)"""
            

    SQ_Shortcut_to_USER_PROFILE = gu.jdbcOracleConnection(query, username, password, connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    ...
    gu.overwriteDeltaPartition(Shortcut_to_WM_USER_PROFILE_PRE, "DC_NBR", dcnbr, target_table_name)
