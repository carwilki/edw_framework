...
def m_WM_Item_Cbo_PRE(dcnbr, env):
    ...
    FROM ITEM_CBO
    WHERE (date_trunc('DD', AUDIT_CREATED_DTTM) >= date_trunc('DD', to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1) OR (date_trunc('DD', AUDIT_LAST_UPDATED_DTTM) >=  date_trunc('DD', to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1) OR (date_trunc('DD', CREATED_DTTM) >= date_trunc('DD', to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1) OR (date_trunc('DD', LAST_UPDATED_DTTM) >=  date_trunc('DD', to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1) AND
    ...
    Shortcut_to_WM_ITEM_CBO_PRE = EXPTRANS.selectExpr( \
        ...
        "CAST(COMMODITY_CLASS_ID AS BIGINT) as COMMODITY_CLASS_ID", \
        ...
        "CAST(BASE_STORAGE_UOM_ID AS BIGINT) as BASE_STORAGE_UOM_ID", \
        ...
        "CAST(UNIT_WEIGHT AS BIGINT) as UNIT_WEIGHT", \
        "CAST(WEIGHT_UOM_ID AS BIGINT) as WEIGHT_UOM_ID", \
        "CAST(UNIT_VOLUME AS BIGINT) as UNIT_VOLUME", \
        "CAST(VOLUME_UOM_ID AS BIGINT) as VOLUME_UOM_ID", \
        "CAST(UNIT_HEIGHT AS BIGINT) as UNIT_HEIGHT", \
        "CAST(UNIT_WIDTH AS BIGINT) as UNIT_WIDTH", \
        "CAST(UNIT_LENGTH AS BIGINT) as UNIT_LENGTH", \
        "CAST(DIMENSION_UOM_ID AS BIGINT) as DIMENSION_UOM_ID", \
        "CAST(VARIABLE_WEIGHT AS BIGINT) as VARIABLE_WEIGHT", \
        "CAST(DATABASE_QTY_UOM_ID AS BIGINT) as DATABASE_QTY_UOM_ID", \
        "CAST(DISPLAY_QTY_UOM_ID AS BIGINT) as DISPLAY_QTY_UOM_ID", \
        "CAST(STATUS_CODE AS BIGINT) as STATUS_CODE", \
        ...
        "CAST(CHANNEL_TYPE_ID AS BIGINT) as CHANNEL_TYPE_ID", \
        "CAST(COLOR_DESC AS STRING) as COLOR_DESC", \
        "CAST(VERSION AS BIGINT) as VERSION", \
        "CAST(ITEM_QUALITY_CODE AS BIGINT) as ITEM_QUALITY_CODE", \
        ...
        "CAST(CAVITY_LEN AS BIGINT) as CAVITY_LEN", \
        "CAST(CAVITY_WD AS BIGINT) as CAVITY_WD", \
        "CAST(CAVITY_HT AS BIGINT) as CAVITY_HT", \
        "CAST(INCREMENTAL_LEN AS BIGINT) as INCREMENTAL_LEN", \
        "CAST(INCREMENTAL_WD AS BIGINT) as INCREMENTAL_WD", \
        "CAST(INCREMENTAL_HT AS BIGINT) as INCREMENTAL_HT", \
        "CAST(STACKABLE_ITEM AS STRING) as STACKABLE_ITEM", \
        "CAST(MAX_NEST_NUMBER AS BIGINT) as MAX_NEST_NUMBER", \
        "CAST(STCC_CODE_ID AS BIGINT) as STCC_CODE_ID", \
        "CAST(SITC_CODE_ID AS BIGINT) as SITC_CODE_ID", \
        "CAST(SCHEDULE_B_CODE_ID AS BIGINT) as SCHEDULE_B_CODE_ID", \
        "CAST(SOLD_ONLINE AS BIGINT) as SOLD_ONLINE", \
        "CAST(SOLD_IN_STORES AS BIGINT) as SOLD_IN_STORES", \
        ...
        "CAST(SELL_THROUGH AS BIGINT) as SELL_THROUGH", \
        "CAST(COMMERCE_ATTRIBUTE1 AS STRING) as COMMERCE_ATTRIBUTE1", \
        "CAST(COMMERCE_ATTRIBUTE2 AS STRING) as COMMERCE_ATTRIBUTE2", \
        "CAST(COMMERCE_NUM_ATTRIBUTE1 AS BIGINT) as COMMERCE_NUM_ATTRIBUTE1", \
        "CAST(COMMERCE_NUM_ATTRIBUTE2 AS BIGINT) as COMMERCE_NUM_ATTRIBUTE2", \
        ...
        "CAST(SIZE_SEQ AS BIGINT) as SIZE_SEQ", \
        "CAST(COLOR_SEQ AS BIGINT) as COLOR_SEQ", \
        "CAST(RETURNABLE_AT_STORE AS BIGINT) as RETURNABLE_AT_STORE", \
    ...
    logger.info(
        "Shortcut_to_WM_ITEM_CBO_PRE is written to the target table - "
        + target_table_name
    )