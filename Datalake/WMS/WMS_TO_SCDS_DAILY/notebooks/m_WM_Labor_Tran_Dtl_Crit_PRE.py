...
def m_WM_Labor_Tran_Dtl_Crit_PRE(dcnbr, env):
    ...
    FROM {source_schema}.LABOR_TRAN_DTL_CRIT
    WHERE (trunc(LABOR_TRAN_DTL_CRIT.CREATED_DTTM) >= trunc(to_date('{Prev_Run_Dt}','YYYY-MM-DD'))-1) OR (trunc(LABOR_TRAN_DTL_CRIT.LAST_UPDATED_DTTM) >=  trunc(to_date('{Prev_Run_Dt}','YYYY-MM-DD'))-1)  AND
    ...
    Shortcut_to_WM_LABOR_TRAN_DTL_CRIT_PRE = EXPTRANS.selectExpr(
        ...
        "CAST(LABOR_TRAN_DTL_CRIT_ID AS DECIMAL(20,0)) as LABOR_TRAN_DTL_CRIT_ID",
        "CAST(LABOR_TRAN_DTL_ID AS DECIMAL(20,0)) as LABOR_TRAN_DTL_ID",
