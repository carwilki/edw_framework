...
def m_WM_Lpn_Detail_PRE(dcnbr, env):
    ...
    FROM LPN_DETAIL
    WHERE (trunc(LPN_DETAIL.CREATED_DTTM) >= trunc(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1) OR (trunc(LPN_DETAIL.LAST_UPDATED_DTTM) >=  trunc(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1)  AND
    ...
    Shortcut_to_WM_LPN_DETAIL_PRE = EXP_TRN.selectExpr( \
        ...
        "CAST(LPN_DETAIL_STATUS AS BIGINT) as LPN_DETAIL_STATUS", \
        ...
        "CAST(PACK_WEIGHT AS BIGINT) as PACK_WEIGHT", \
        ...
        "CAST(ASSIGNED_QTY AS BIGINT) as ASSIGNED_QTY", \
        "CAST(PREPACK_GROUP_CODE AS STRING) as PREPACK_GROUP_CODE", \
        "CAST(PACK_CODE AS BIGINT) as PACK_CODE", \
        "CAST(SHIPPED_QTY AS BIGINT) as SHIPPED_QTY", \
        "CAST(INITIAL_QTY AS BIGINT) as INITIAL_QTY", \
        ...
        "CAST(HAZMAT_QTY AS BIGINT) as HAZMAT_QTY", \
