...
# print("Prev_Run_Dt is " + Prev_Run_Dt)
# COMMAND ----------
...
# print(f"Prev_Run_Dt is {Prev_Run_Dt}\n")
...
EXP_OUTPUT_VALIDATOR = FIL_UNCHANGED_RECORDS_temp.selectExpr( \
	...
	f"IF(FIL_UNCHANGED_RECORDS___ELM_ID IS NOT NULL AND FIL_UNCHANGED_RECORDS___i_WM_ELM_ID IS NULL, 'INSERT', \
	   IF(FIL_UNCHANGED_RECORDS___ELM_ID IS NULL AND FIL_UNCHANGED_RECORDS___i_WM_ELM_ID IS NOT NULL AND \
			( FIL_UNCHANGED_RECORDS___v_i_WM_CREATE_TSTMP >= DATE_ADD('{Prev_Run_Dt}' ,-14 ) \
			  OR FIL_UNCHANGED_RECORDS___v_i_WM_MOD_TSTMP >= DATE_ADD( '{Prev_Run_Dt}',-14 )), 'DELETE',\
			  IF(FIL_UNCHANGED_RECORDS___ELM_ID IS NOT NULL AND FIL_UNCHANGED_RECORDS___i_WM_ELM_ID IS NOT NULL \
			  AND ( FIL_UNCHANGED_RECORDS___v_i_WM_CREATE_TSTMP <> FIL_UNCHANGED_RECORDS___v_CREATE_DATE_TIME \
			  OR FIL_UNCHANGED_RECORDS___v_i_WM_MOD_TSTMP <> FIL_UNCHANGED_RECORDS___v_MOD_DATE_TIME ), 'UPDATE', NULL))) as o_UPDATE_VALIDATOR" \
)
...
Shortcut_to_WM_E_ELM_CRIT1 = UPD_INS_UPD.selectExpr(
...
Shortcut_to_WM_E_ELM_CRIT11.createOrReplaceTempView('WM_E_ELM_CRIT_DEL')

spark.sql(f"""
          MERGE INTO {refined_perf_table} trg
          USING WM_E_ELM_CRIT_DEL src
          ON (src.LOCATION_ID = trg.LOCATION_ID AND src.WM_ELM_ID = trg.WM_ELM_ID AND src.WM_CRIT_ID = trg.WM_CRIT_ID AND src.WM_CRIT_VAL_ID = trg.WM_CRIT_VAL_ID )
