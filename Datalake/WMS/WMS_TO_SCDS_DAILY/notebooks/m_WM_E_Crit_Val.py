...
f"IF(FIL_UNCHANGED_RECORDS___CRIT_VAL_ID IS NOT NULL AND FIL_UNCHANGED_RECORDS___i_WM_CRIT_VAL_ID IS NULL, 'INSERT', IF(FIL_UNCHANGED_RECORDS___CRIT_VAL_ID IS NULL AND FIL_UNCHANGED_RECORDS___i_WM_CRIT_VAL_ID IS NOT NULL AND ( FIL_UNCHANGED_RECORDS___v_i_WM_CREATE_TSTMP >= DATE_ADD('{Prev_Run_Dt}',-14) OR FIL_UNCHANGED_RECORDS___v_i_WM_MOD_TSTMP >= DATE_ADD('{Prev_Run_Dt}',-14) ), 'DELETE', IF(FIL_UNCHANGED_RECORDS___CRIT_VAL_ID IS NOT NULL AND FIL_UNCHANGED_RECORDS___i_WM_CRIT_VAL_ID IS NOT NULL AND ( FIL_UNCHANGED_RECORDS___v_i_WM_CREATE_TSTMP <> FIL_UNCHANGED_RECORDS___v_CREATE_DATE_TIME OR FIL_UNCHANGED_RECORDS___v_i_WM_MOD_TSTMP <> FIL_UNCHANGED_RECORDS___v_MOD_DATE_TIME ), 'UPDATE', NULL))) as o_UPDATE_VALIDATOR" \
)
...
Shortcut_to_WM_E_CRIT_VAL11.createOrReplaceTempView('WM_E_CRIT_VAL_DEL')

spark.sql(f"""
          MERGE INTO {refined_perf_table} trg
          USING WM_E_CRIT_VAL_DEL src
          ON (src.LOCATION_ID = trg.LOCATION_ID AND src.WM_CRIT_VAL_ID = trg.WM_CRIT_VAL_ID AND src.WM_CRIT_ID = trg.WM_CRIT_ID )
