...
def m_WM_E_Aud_Log_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_E_AUD_LOG = gu.jdbcOracleConnection(
        f"""SELECT
                E_AUD_LOG.AUD_ID,
                E_AUD_LOG.WHSE,
                E_AUD_LOG.TRAN_NBR,
            ...
            FROM E_AUD_LOG
            WHERE (TRUNC( E_AUD_LOG.CREATE_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14)""",
        username,
        password,
        connection_string,
    ...
    SQ_Shortcut_to_E_AUD_LOG_temp = SQ_Shortcut_to_E_AUD_LOG.toDF(
        *[
            "SQ_Shortcut_to_E_AUD_LOG___" + col
            for col in SQ_Shortcut_to_E_AUD_LOG.columns
        ]
    )
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_E_AUD_LOG_PRE, "DC_NBR", dcnbr, target_table_name
    )
