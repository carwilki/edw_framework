...
def m_WM_Task_Hdr_PRE(dcnbr, env):
    ...
    query = f"""SELECT
                ...
                FROM {source_schema}.TASK_HDR
                WHERE  (TRUNC(TASK_HDR.CREATE_DATE_TIME)>= TRUNC(to_date('{Prev_Run_Dt}','YYYY-MM-DD')) - 1) OR (TRUNC(TASK_HDR.MOD_DATE_TIME)>= TRUNC(to_date('{Prev_Run_Dt}','YYYY-MM-DD')) - 1)"""
            

    SQ_Shortcut_to_TASK_HDR = jdbcOracleConnection(query, username, password, connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    ...
    Shortcut_to_WM_TASK_HDR_PRE = EXP_TRN.selectExpr(
        ...
        "CAST(DFLT_TASK_PRTY AS INT) as DFLT_TASK_PRTY",
        "CAST(CURR_TASK_PRTY AS INT) as CURR_TASK_PRTY",
        "CAST(XPECTD_DURTN AS INT) as XPECTD_DURTN",
        "CAST(ACTL_DURTN AS INT) as ACTL_DURTN",
        ...
        "CAST(TASK_PARM_ID AS INT) as TASK_PARM_ID",
        ...
        "CAST(ORIG_TASK_ID AS INT) as ORIG_TASK_ID",
        ...
        "CAST(TASK_RULE_PARM_ID AS INT) as TASK_RULE_PARM_ID",
        "CAST(PICK_CART_TYPE AS STRING) as PICK_CART_TYPE",
        "CAST(REPRINT_COUNT AS TINYINT) as REPRINT_COUNT",
        "CAST(ESTIMATED_TIME AS DECIMAL(16,4)) as ESTIMATED_TIME",
        "CAST(ESTIMATED_DISTANCE AS DECIMAL(16,4)) as ESTIMATED_DISTANCE",
        "CAST(LOAD_TSTMP_EXP AS TIMESTAMP) as LOAD_TSTMP"
    )    
    overwriteDeltaPartition(Shortcut_to_WM_TASK_HDR_PRE, "DC_NBR", dcnbr, target_table_name)
