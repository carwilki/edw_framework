...
def m_WM_E_Job_Function_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_E_JOB_FUNCTION = gu.jdbcOracleConnection(
        f"""SELECT
    ...
    SQ_Shortcut_to_E_JOB_FUNCTION_temp = SQ_Shortcut_to_E_JOB_FUNCTION.toDF(
        *[
            "SQ_Shortcut_to_E_JOB_FUNCTION___" + col
            for col in SQ_Shortcut_to_E_JOB_FUNCTION.columns
        ]
    )
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_E_JOB_FUNCTION_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
