...
SQ_Shortcut_to_WM_YARD_ZONE_PRE = spark.sql(
    f"""SELECT
...
SQ_Shortcut_to_WM_YARD_ZONE = spark.sql(
    f"""SELECT
...
FROM WM_YARD_ZONE
WHERE {Del_Logic} 1=0 and DELETE_FLAG =0"""
).withColumn("sys_row_id", monotonically_increasing_id())
...
SQ_Shortcut_to_WM_YARD_ZONE_PRE_temp = SQ_Shortcut_to_WM_YARD_ZONE_PRE.toDF(
    *[
        "SQ_Shortcut_to_WM_YARD_ZONE_PRE___" + col
        for col in SQ_Shortcut_to_WM_YARD_ZONE_PRE.columns
    ]
)
...
SQ_Shortcut_to_WM_YARD_ZONE_temp = SQ_Shortcut_to_WM_YARD_ZONE.toDF(
    *[
        "SQ_Shortcut_to_WM_YARD_ZONE___" + col
        for col in SQ_Shortcut_to_WM_YARD_ZONE.columns
    ]
)

JNR_WM_YARD_ZONE = SQ_Shortcut_to_WM_YARD_ZONE_temp.join(
    JNR_SITE_PROFILE_temp,
    [
        SQ_Shortcut_to_WM_YARD_ZONE_temp.SQ_Shortcut_to_WM_YARD_ZONE___LOCATION_ID
        == JNR_SITE_PROFILE_temp.JNR_SITE_PROFILE___LOCATION_ID,
        SQ_Shortcut_to_WM_YARD_ZONE_temp.SQ_Shortcut_to_WM_YARD_ZONE___WM_YARD_ID
        == JNR_SITE_PROFILE_temp.JNR_SITE_PROFILE___YARD_ID,
        SQ_Shortcut_to_WM_YARD_ZONE_temp.SQ_Shortcut_to_WM_YARD_ZONE___WM_YARD_ZONE_ID
        == JNR_SITE_PROFILE_temp.JNR_SITE_PROFILE___YARD_ZONE_ID,
    ],
    "fullouter",
).selectExpr(
...
# for each involved DataFrame, append the dataframe name to each column
JNR_WM_YARD_ZONE_temp = JNR_WM_YARD_ZONE.toDF(
    *["JNR_WM_YARD_ZONE___" + col for col in JNR_WM_YARD_ZONE.columns]
)

FIL_UNCHANGED_RECORDS = (
    JNR_WM_YARD_ZONE_temp.selectExpr(
        ...
        "JNR_WM_YARD_ZONE___i_LOAD_TSTMP as i_LOAD_TSTMP",
    )
    .filter(
        expr(
            "YARD_ZONE_ID IS NULL OR i_WM_YARD_ZONE_ID IS NULL OR (NOT i_WM_YARD_ZONE_ID IS NULL AND (COALESCE(CREATED_DTTM, date'1900-01-01') != COALESCE(i_WM_CREATED_TSTMP, date'1900-01-01')) OR (COALESCE(LAST_UPDATED_DTTM, date'1900-01-01') != COALESCE(i_WM_LAST_UPDATED_TSTMP, date'1900-01-01'))))"
        )
    )
    .withColumn("sys_row_id", monotonically_increasing_id())
)
...
# for each involved DataFrame, append the dataframe name to each column
FIL_UNCHANGED_RECORDS_temp = (
    FIL_UNCHANGED_RECORDS.toDF(
        *["FIL_UNCHANGED_RECORDS___" + col for col in FIL_UNCHANGED_RECORDS.columns]
    )
    .withColumn(
        "v_CREATED_DTTM",
        expr("""IF (CREATED_DTTM IS NULL, date'1900-01-01', CREATED_DTTM)"""),
    )
    .withColumn(
        "v_LAST_UPDATED_DTTM",
        expr("""IF (LAST_UPDATED_DTTM IS NULL, date'1900-01-01', LAST_UPDATED_DTTM)"""),
    )
    .withColumn(
        "v_i_WM_CREATED_TSTMP",
        expr(
            """IF (i_WM_CREATED_TSTMP IS NULL, date'1900-01-01', i_WM_CREATED_TSTMP)"""
        ),
    )
    .withColumn(
        "v_i_WM_LAST_UPDATED_TSTMP",
        expr(
            """IF (i_WM_LAST_UPDATED_TSTMP IS NULL, date'1900-01-01', i_WM_LAST_UPDATED_TSTMP)"""
        ),
    )
)
...
RTR_INS_UPD_DEL_DELETE = EXP_UPDATE_VALIDATOR.select(
    EXP_UPDATE_VALIDATOR.sys_row_id.alias("sys_row_id"),
    ...
    EXP_UPDATE_VALIDATOR.i_WM_LAST_UPDATED_SOURCE_TYPE.alias(
        "i_WM_LAST_UPDATED_SOURCE_TYPE3"
    ),
...
).filter("o_UPDATE_VALIDATOR = 'DELETE'")

# Creating output dataframe for RTR_INS_UPD_DEL, output group INSERT_UPDATE
RTR_INS_UPD_DEL_INSERT_UPDATE = EXP_UPDATE_VALIDATOR.select(
    EXP_UPDATE_VALIDATOR.sys_row_id.alias("sys_row_id"),
    ...
    EXP_UPDATE_VALIDATOR.i_WM_LAST_UPDATED_SOURCE_TYPE.alias(
        "i_WM_LAST_UPDATED_SOURCE_TYPE1"
    ),
...
).filter("o_UPDATE_VALIDATOR = 'INSERT' OR o_UPDATE_VALIDATOR = 'UPDATE'")
...
# for each involved DataFrame, append the dataframe name to each column
RTR_INS_UPD_DEL_INSERT_UPDATE_temp = RTR_INS_UPD_DEL_INSERT_UPDATE.toDF(
    *[
        "RTR_INS_UPD_DEL_INSERT_UPDATE___" + col
        for col in RTR_INS_UPD_DEL_INSERT_UPDATE.columns
    ]
)
...
UPD_INS_UPD = RTR_INS_UPD_DEL_INSERT_UPDATE_temp.selectExpr(
    ...
    "pyspark_data_action",
    when(
        RTR_INS_UPD_DEL_INSERT_UPDATE.o_UPDATE_VALIDATOR1 == (lit("INSERT")), lit(0)
    ).when(
        RTR_INS_UPD_DEL_INSERT_UPDATE.o_UPDATE_VALIDATOR1 == (lit("UPDATE")), lit(1)
    ),
)
...
# for each involved DataFrame, append the dataframe name to each column
RTR_INS_UPD_DEL_DELETE_temp = RTR_INS_UPD_DEL_DELETE.toDF(
    *["RTR_INS_UPD_DEL_DELETE___" + col for col in RTR_INS_UPD_DEL_DELETE.columns]
)
...
try:
    ...
    logPrevRunDt(
        "WM_YARD_ZONE", "WM_YARD_ZONE", "Completed", "N/A", f"{raw}.log_run_details"
    )
