...
refined_perf_table = f"{refine}.WM_ITEM_PACKAGE_CBO"
...
EXP_INT_CONV = SQ_Shortcut_to_WM_ITEM_PACKAGE_CBO_PRE_temp.selectExpr( \
	...
	"cast(SQ_Shortcut_to_WM_ITEM_PACKAGE_CBO_PRE___DC_NBR as int) as DC_NBR", \
...
EXP_EVALUATE = FIL_NO_CHANGE_REC_temp.selectExpr( \
    ...
    "CASE WHEN TRIM(UPPER(FIL_NO_CHANGE_REC___IS_STD)) IN ('Y', '1') THEN '1' ELSE '0' END as IS_STD1_EXP", \
...
UPD_INS_UPD = EXP_EVALUATE_temp.selectExpr( \
	...
	.withColumn('pyspark_data_action', when((col('in_WM_ITEM_PACKAGE_ID').isNull()) ,(lit(0))).otherwise(lit(1)))
...
Shortcut_to_WM_ITEM_PACKAGE_CBO = UPD_INS_UPD.selectExpr(
	...
	"CAST(ITEM_PACKAGE_ID AS INT) as WM_ITEM_PACKAGE_ID",
	"CAST(ITEM_ID AS INT) as WM_ITEM_ID",
	"CAST(PACKAGE_UOM_ID AS INT) as WM_PACKAGE_UOM_ID",
	"CAST(GTIN1 AS STRING) as GTIN",
	"CAST(BUSINESS_PARTNER_ID AS STRING) as WM_BUSINESS_PARTNER_ID",
	"CAST(IS_STD1 AS TINYINT) as STANDARD_UOM_FLAG",
	"CAST(QUANTITY AS DECIMAL(16,4)) as ITEM_PACKAGE_QTY",
	"CAST(WEIGHT AS DECIMAL(16,4)) as ITEM_PACKAGE_WEIGHT",
	"CAST(WEIGHT_UOM_ID AS INT) as WM_WEIGHT_UOM_ID",
	"CAST(VOLUME AS DECIMAL(16,4)) as ITEM_PACKAGE_VOLUME",
	"CAST(VOLUME_UOM_ID AS DECIMAL(16,4)) as WM_VOLUME_UOM_ID",
	"CAST(LENGTH AS DECIMAL(16,4)) as ITEM_PACKAGE_LENGTH",
	"CAST(HEIGHT AS DECIMAL(16,4)) as ITEM_PACKAGE_HEIGHT",
	"CAST(WIDTH AS DECIMAL(16,4)) as ITEM_PACKAGE_WIDTH",
	...
	"CAST(AUDIT_CREATED_SOURCE_TYPE AS TINYINT) as WM_CREATED_SOURCE_TYPE",
	"CAST(AUDIT_CREATED_SOURCE AS STRING) as WM_CREATED_SOURCE",
	"CAST(AUDIT_CREATED_DTTM AS TIMESTAMP) as WM_CREATED_TSTMP",
	"CAST(AUDIT_LAST_UPDATED_SOURCE_TYPE AS TINYINT) as WM_LAST_UPDATED_SOURCE_TYPE",
