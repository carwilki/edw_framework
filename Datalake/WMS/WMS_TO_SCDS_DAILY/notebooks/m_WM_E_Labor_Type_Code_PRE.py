...
def m_WM_E_Labor_Type_Code_PRE(dcnbr, env):
    ...
    refine_table_name = "E_LABOR_TYPE_CODE"
    ...
    SQ_Shortcut_to_E_LABOR_TYPE_CODE = gu.jdbcOracleConnection(
        f"""SELECT
            ...
            WHERE (TRUNC( CREATE_DATE_TIME)>= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (TRUNC( MOD_DATE_TIME)>= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (TRUNC( CREATED_DTTM)>= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (TRUNC( LAST_UPDATED_DTTM)>= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14)""",
        username,
        password,
        connection_string,
    ...
    SQ_Shortcut_to_E_LABOR_TYPE_CODE_temp = SQ_Shortcut_to_E_LABOR_TYPE_CODE.toDF(
        *[
            "SQ_Shortcut_to_E_LABOR_TYPE_CODE___" + col
            for col in SQ_Shortcut_to_E_LABOR_TYPE_CODE.columns
        ]
    )
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_E_LABOR_TYPE_CODE_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
