...
def m_WM_E_Act_Elm_Crit_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_E_ACT_ELM_CRIT = gu.jdbcOracleConnection(
        f"""SELECT
            ...
            FROM E_ACT_ELM_CRIT
            WHERE (TRUNC( E_ACT_ELM_CRIT.CREATE_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (TRUNC( E_ACT_ELM_CRIT.MOD_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14)""",
        username,
        password,
        connection_string,
    ...
    SQ_Shortcut_to_E_ACT_ELM_CRIT_temp = SQ_Shortcut_to_E_ACT_ELM_CRIT.toDF(
        *[
            "SQ_Shortcut_to_E_ACT_ELM_CRIT___" + col
            for col in SQ_Shortcut_to_E_ACT_ELM_CRIT.columns
        ]
    )
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_E_ACT_ELM_CRIT_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
