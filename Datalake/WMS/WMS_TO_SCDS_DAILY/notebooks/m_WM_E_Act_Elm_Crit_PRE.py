...
def m_WM_E_Act_Elm_Crit_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_E_ACT_ELM_CRIT = jdbcOracleConnection(
        f"""SELECT
            ...
            FROM {source_schema}.E_ACT_ELM_CRIT
            WHERE (TRUNC( E_ACT_ELM_CRIT.CREATE_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','YYYY-MM-DD')) - 14) OR (TRUNC( E_ACT_ELM_CRIT.MOD_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','YYYY-MM-DD')) - 14)""",username,password,connection_string).withColumn("sys_row_id", monotonically_increasing_id())
