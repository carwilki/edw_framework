...
def m_WM_Outpt_Lpn_Detail_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_OUTPT_LPN_DETAIL = jdbcOracleConnection(
        query, username, password, connection_string
    ).withColumn("sys_row_id", monotonically_increasing_id())
    logger.info(
        "SQL query for SQ_Shortcut_to_OUTPT_LPN_DETAIL is executed and data is loaded using jdbc"
    )
    ...
    SQ_Shortcut_to_OUTPT_LPN_DETAIL_temp = SQ_Shortcut_to_OUTPT_LPN_DETAIL.toDF(
        *[
            "SQ_Shortcut_to_OUTPT_LPN_DETAIL___" + col
            for col in SQ_Shortcut_to_OUTPT_LPN_DETAIL.columns
        ]
    )
    ...
    overwriteDeltaPartition(
        Shortcut_to_WM_OUTPT_LPN_DETAIL_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
        "Shortcut_to_WM_OUTPT_LPN_DETAIL_PRE is written to the target table - "
        + target_table_name
    )
