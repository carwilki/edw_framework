...
# soft_delete_logic_Wm_E_Evnt_Smry_Hdr= ' $$soft_delete_logic_Wm_E_Evnt_Smry_Hdr=
# update Wm_E_Evnt_Smry_Hdr set delete_flag=1 , update_tstmp=current_date where (location_id, WM_ELS_TRAN_ID) 
# in (select location_id, WM_ELS_TRAN_ID from  Wm_E_Evnt_Smry_Hdr base_table 
# where base_table.DELETE_FLAG=0 and (cast(base_table.WM_CREATE_tstmp as date) >= (cast('$$Prev_Run_Dt' as date )-14) or cast(base_table.WM_MOD_TSTMP as date) >= (cast('$$Prev_Run_Dt' as date )-14)) 
# and (location_id, WM_ELS_TRAN_ID) not in (select   SP.location_id, pre_table.ELS_TRAN_ID from  Wm_E_Evnt_Smry_Hdr_pre pre_table,site_profile SP where SP.store_nbr =pre_table.dc_nbr));
# ' # args.soft_delete_logic_Wm_E_Evnt_Smry_Hdr

# update wm_trailer_contents set delete_flag=1, update_tstmp=current_timestamp where
soft_delete_logic_Wm_E_Evnt_Smry_Hdr= f""" 
select  location_id, WM_ELS_TRAN_ID 
from {refined_perf_table}  
where (location_id, WM_ELS_TRAN_ID) in 
(select location_id, WM_ELS_TRAN_ID 
...
and (location_id, WM_ELS_TRAN_ID) not in 
( select   SP.location_id, pre_table.ELS_TRAN_ID 
from  {raw_perf_table} pre_table, {site_profile_table} SP 
where SP.store_nbr =pre_table.dc_nbr))"""

sd_df = spark.sql(soft_delete_logic_Wm_E_Evnt_Smry_Hdr)

sd_df.createOrReplaceTempView('WM_E_EVNT_SMRY_HDR_SD')

spark.sql(f"""
          MERGE INTO {refined_perf_table} tgt
          USING  WM_E_EVNT_SMRY_HDR_SD src
          ON src.location_id = tgt.location_id and src.WM_ELS_TRAN_ID = tgt.WM_ELS_TRAN_ID
