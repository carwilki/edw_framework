...
def m_WM_C_TMS_Plan_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_C_TMS_PLAN = gu.jdbcOracleConnection(
        f"""SELECT
    ...
    SQ_Shortcut_to_C_TMS_PLAN_temp = SQ_Shortcut_to_C_TMS_PLAN.toDF(
        *[
            "SQ_Shortcut_to_C_TMS_PLAN___" + col
            for col in SQ_Shortcut_to_C_TMS_PLAN.columns
        ]
    )
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_C_TMS_PLAN_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
