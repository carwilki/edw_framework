...
def m_WM_Sys_Code_PRE(dcnbr, env):
    ...
    query = f"""SELECT
                ...
                FROM SYS_CODE
                WHERE {Initial_Load} (TRUNC(SYS_CODE.CREATE_DATE_TIME)>= TRUNC(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (TRUNC(SYS_CODE.MOD_DATE_TIME)>= TRUNC(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (TRUNC(SYS_CODE.CREATED_DTTM)>= TRUNC(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (TRUNC(SYS_CODE.LAST_UPDATED_DTTM)>= TRUNC(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14)"""
    

    SQ_Shortcut_to_SYS_CODE = gu.jdbcOracleConnection(query, username, password, connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    ...
    Shortcut_to_WM_SYS_CODE_PRE = EXPTRANS.selectExpr( 
    	...
    	"CAST(SYS_CODE_ID AS BIGINT) as SYS_CODE_ID", 
    	"CAST(SYS_CODE_TYPE_ID AS BIGINT) as SYS_CODE_TYPE_ID", 
    	"CAST(CREATED_DTTM AS TIMESTAMP) as CREATED_DTTM", 
    	"CAST(LAST_UPDATED_DTTM AS TIMESTAMP) as LAST_UPDATED_DTTM", 
    	"CAST(LOAD_TSTMP_EXP AS TIMESTAMP) as LOAD_TSTMP" 
    )
    
    gu.overwriteDeltaPartition(Shortcut_to_WM_SYS_CODE_PRE, "DC_NBR", dcnbr, target_table_name)
