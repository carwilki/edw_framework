...
def m_WM_Labor_Msg_PRE(dcnbr, env):
    ...
    FROM LABOR_MSG
    WHERE (trunc(LABOR_MSG.CREATED_DTTM) >= trunc(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1) OR (trunc(LABOR_MSG.LAST_UPDATED_DTTM) >=  trunc(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1)  AND
