...
def m_WM_Labor_Msg_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_LABOR_MSG = jdbcOracleConnection(
        f"""SELECT
    ...
    cast(LABOR_MSG.SCHED_START_TIME as timestamp) as SCHED_START_TIME,
    cast(LABOR_MSG.SCHED_START_DATE as timestamp) as SCHED_START_DATE,
    LABOR_MSG.VHCL_TYPE,
    LABOR_MSG.REF_CODE,
    LABOR_MSG.REF_NBR,
    ...
    cast(LABOR_MSG.CREATED_DTTM as timestamp) as CREATED_DTTM,
    LABOR_MSG.LAST_UPDATED_SOURCE_TYPE,
    LABOR_MSG.LAST_UPDATED_SOURCE,
    cast(LABOR_MSG.LAST_UPDATED_DTTM as timestamp) as LAST_UPDATED_DTTM,
    cast(LABOR_MSG.ACTUAL_END_TIME as timestamp) as ACTUAL_END_TIME, 
    cast(LABOR_MSG.ACTUAL_END_DATE as timestamp) as ACTUAL_END_DATE,
    LABOR_MSG.PLAN_ID,
    LABOR_MSG.WAVE_NBR,
    ...
    LABOR_MSG.TOTAL_QTY,
    cast(LABOR_MSG.ORIG_EVNT_START_TIME as timestamp) as ORIG_EVNT_START_TIME,
    cast(LABOR_MSG.ORIG_EVNT_END_TIME as timestamp) as ORIG_EVNT_END_TIME,
    ...
    SQ_Shortcut_to_LABOR_MSG_temp = SQ_Shortcut_to_LABOR_MSG.toDF(
        *[
            "SQ_Shortcut_to_LABOR_MSG___" + col
            for col in SQ_Shortcut_to_LABOR_MSG.columns
        ]
    )
    ...
    Shortcut_to_WM_LABOR_MSG_PRE = EXPTRANS.selectExpr(
        ...
        "CAST(TRAN_DATA_ELS_TRAN_ID AS DECIMAL(20,0)) as TRAN_DATA_ELS_TRAN_ID",
        ...
        "CAST(MSG_STAT_CODE AS SMALLINT) as MSG_STAT_CODE",
        "CAST(MONITOR_SMRY_ID AS INT) as MONITOR_SMRY_ID",
        "CAST(ORIG_ACT_NAME AS STRING) as ORIG_ACT_NAME",
        "CAST(SOURCE AS STRING) as SOURCE",
        "CAST(HAS_MONITOR_MSG AS TINYINT) as HAS_MONITOR_MSG",
    ...
    overwriteDeltaPartition(
        Shortcut_to_WM_LABOR_MSG_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
