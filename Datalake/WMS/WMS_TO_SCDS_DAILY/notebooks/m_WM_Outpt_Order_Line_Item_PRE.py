...
def m_WM_Outpt_Order_Line_Item_PRE(dcnbr, env):
    ...
    refine_table_name = "WM_OUTPT_ORDER_LINE_ITEM"
    ...
    query = f"""SELECT
                ...
                WHERE {Initial_Load} (trunc(OUTPT_ORDER_LINE_ITEM.CREATED_DTTM) >= trunc(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1) OR (trunc(OUTPT_ORDER_LINE_ITEM.LAST_UPDATED_DTTM) >=  trunc(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1)"""

    SQ_Shortcut_to_OUTPT_ORDER_LINE_ITEM = gu.jdbcOracleConnection(query, username, password, connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    ...
    gu.overwriteDeltaPartition(Shortcut_to_WM_OUTPT_ORDER_LINE_ITEM_PRE, "DC_NBR", dcnbr, target_table_name)
