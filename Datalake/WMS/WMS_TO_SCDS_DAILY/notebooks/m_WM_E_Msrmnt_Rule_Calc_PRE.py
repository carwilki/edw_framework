...
def m_WM_E_Msrmnt_Rule_Calc_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_E_MSRMNT_RULE_CALC = jdbcOracleConnection(
        f"""SELECT
            ...
            FROM {source_schema}.E_MSRMNT_RULE_CALC
            WHERE (TRUNC( E_MSRMNT_RULE_CALC.CREATE_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','YYYY-MM-DD')) - 14) OR (TRUNC( E_MSRMNT_RULE_CALC.MOD_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','YYYY-MM-DD')) - 14)""",username,password,connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    ...
    SQ_Shortcut_to_E_MSRMNT_RULE_CALC_temp = SQ_Shortcut_to_E_MSRMNT_RULE_CALC.toDF(*["SQ_Shortcut_to_E_MSRMNT_RULE_CALC___" + col for col in SQ_Shortcut_to_E_MSRMNT_RULE_CALC.columns])
    ...
    Shortcut_to_WM_E_MSRMNT_RULE_CALC_PRE = EXPTRANS.selectExpr(
        ...
        "CAST(CALC_SEQ_NBR AS INT) as CALC_SEQ_NBR",
    ...
    overwriteDeltaPartition(Shortcut_to_WM_E_MSRMNT_RULE_CALC_PRE,"DC_NBR",dcnbr,target_table_name)
    logger.info(
