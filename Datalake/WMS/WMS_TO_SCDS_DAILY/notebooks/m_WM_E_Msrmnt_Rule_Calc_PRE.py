...
def m_WM_E_Msrmnt_Rule_Calc_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_E_MSRMNT_RULE_CALC = jdbcOracleConnection(
        f"""SELECT
            ...
            WHERE (TRUNC( E_MSRMNT_RULE_CALC.CREATE_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (TRUNC( E_MSRMNT_RULE_CALC.MOD_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14)""",username,password,connection_string).withColumn("sys_row_id", monotonically_increasing_id())
