...
def m_WM_Lpn_PRE(dcnbr, env):
    ...
    tableName = "WM_LPN_PRE', mode = 'append"
    schemaName = raw
    
    target_table_name = schemaName + "." + tableName
    refine_table_name = "WM_LPN', mode = 'append"
    ...
    query = f"""SELECT
                    ...
                    FROM LPN
                    WHERE {Initial_Load} (date_trunc('DD', LPN.CREATED_DTTM) >= date_trunc('DD', to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 1) OR (date_trunc('DD', LPN.EXT_CREATED_DTTM) >= date_trunc('DD', to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 1) OR (date_trunc('DD', LPN.LAST_UPDATED_DTTM) >= date_trunc('DD', to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 1)"""

    SQ_Shortcut_to_LPN = gu.jdbcOracleConnection(query, username, password, connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    ...
    Shortcut_to_WM_LPN_PRE = EXPTRANS.selectExpr( \
    	...
    	"CAST(ROUTE_RULE_ID AS BIGINT) as ROUTE_RULE_ID", \
    	...
    	"CAST(RECEIPT_VARIANCE_INDICATOR AS BIGINT) as RECEIPT_VARIANCE_INDICATOR", \
    	...
    	"CAST(ERROR_INDICATOR AS BIGINT) as ERROR_INDICATOR", \
    	"CAST(WARNING_INDICATOR AS BIGINT) as WARNING_INDICATOR", \
    	...
    	"CAST(WAVE_SEQ_NBR AS BIGINT) as WAVE_SEQ_NBR", \
    	"CAST(WAVE_STAT_CODE AS BIGINT) as WAVE_STAT_CODE", \
    	"CAST(DIRECTED_QTY AS BIGINT) as DIRECTED_QTY", \
    	"CAST(TRANSITIONAL_INVENTORY_TYPE AS BIGINT) as TRANSITIONAL_INVENTORY_TYPE", \
    	...
    	"CAST(SELECTION_RULE_ID AS BIGINT) as SELECTION_RULE_ID", \
    	...
    	"CAST(LPN_CREATION_CODE AS BIGINT) as LPN_CREATION_CODE", \
    	...
    	"CAST(LOAD_SEQUENCE AS BIGINT) as LOAD_SEQUENCE", \
    	...
    	"CAST(PRINTING_RULE_ID AS BIGINT) as PRINTING_RULE_ID", \
    	...
    	"CAST(IS_ZONE_SKIPPED AS BIGINT) as IS_ZONE_SKIPPED", \
    	"CAST(ZONESKIP_HUB_FACILITY_ALIAS_ID AS STRING) as ZONESKIP_HUB_FACILITY_ALIAS_ID", \
    	"CAST(ZONESKIP_HUB_FACILITY_ID AS BIGINT) as ZONESKIP_HUB_FACILITY_ID", \
    	"CAST(PLANING_DEST_FACILITY_ALIAS_ID AS BIGINT) as PLANING_DEST_FACILITY_ALIAS_ID", \
    	"CAST(PLANING_DEST_FACILITY_ID AS BIGINT) as PLANING_DEST_FACILITY_ID", \
    	...
    	"CAST(DISTRIBUTION_LEG_MODE_ID AS BIGINT) as DISTRIBUTION_LEG_MODE_ID", \
    	"CAST(DISTRIBUTION_LEV_SVCE_LEVEL_ID AS BIGINT) as DISTRIBUTION_LEV_SVCE_LEVEL_ID", \
    	"CAST(SERVICE_LEVEL_INDICATOR AS STRING) as SERVICE_LEVEL_INDICATOR", \
    	"CAST(COD_FLAG AS BIGINT) as COD_FLAG", \
    	"CAST(COD_AMOUNT AS BIGINT) as COD_AMOUNT", \
    	"CAST(COD_PAYMENT_METHOD AS BIGINT) as COD_PAYMENT_METHOD", \
    	...
    	"CAST(RECEIPT_TYPE AS BIGINT) as RECEIPT_TYPE", \
    	"CAST(VARIANCE_TYPE AS BIGINT) as VARIANCE_TYPE", \
    	...
    	"CAST(OVERPACK AS BIGINT) as OVERPACK", \
    	...
    	"CAST(ALL_PAKD_IN_ONE AS BIGINT) as ALL_PAKD_IN_ONE", \
    	...
    	"CAST(DISPOSITION_SOURCE_ID AS BIGINT) as DISPOSITION_SOURCE_ID", \
    ...
    gu.overwriteDeltaPartition(Shortcut_to_WM_LPN_PRE, "DC_NBR", dcnbr, target_table_name)
