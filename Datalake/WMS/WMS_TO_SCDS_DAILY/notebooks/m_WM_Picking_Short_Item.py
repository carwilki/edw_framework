...
refined_perf_table = f"{refine}.WM_PICKING_SHORT_ITEM"
raw_perf_table = f"{raw}.WM_PICKING_SHORT_ITEM_PRE"
...
LOCATION_ID,
WM_PICKING_SHORT_ITEM_ID,
WM_CREATED_TSTMP,
WM_LAST_UPDATED_TSTMP,
LOAD_TSTMP
FROM {refined_perf_table}
WHERE WM_PICKING_SHORT_ITEM_ID in (SELECT PICKING_SHORT_ITEM_ID FROM {raw_perf_table})""").withColumn("sys_row_id", monotonically_increasing_id())
...
Shortcut_to_WM_PICKING_SHORT_ITEM1 = UPD_INSERT_UPDATE.selectExpr( \
	"CAST(LOCATION_ID1 AS BIGINT) as LOCATION_ID", \
	"CAST(PICKING_SHORT_ITEM_ID AS BIGINT) as WM_PICKING_SHORT_ITEM_ID", \
	...
	"CAST(SHORT_QTY AS BIGINT) as WM_SHORT_QTY", \
	"CAST(STAT_CODE AS BIGINT) as WM_STAT_CODE", \
	"CAST(TC_COMPANY_ID AS BIGINT) as WM_TC_COMPANY_ID", \
	"CAST(SHORT_TYPE AS STRING) as WM_SHORT_TYPE", \
	...
	"CAST(CREATED_DTTM AS DATE) as WM_CREATED_TSTMP", \
	"CAST(CREATED_SOURCE AS STRING) as WM_CREATED_SOURCE", \
	"CAST(LAST_UPDATED_DTTM AS DATE) as WM_LAST_UPDATED_TSTMP", \
...
try:
  ...
  executeMerge(Shortcut_to_WM_PICKING_SHORT_ITEM1, refined_perf_table, primary_key)
