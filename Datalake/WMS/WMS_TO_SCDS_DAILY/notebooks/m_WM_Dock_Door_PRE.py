...
def m_WM_Dock_Door_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_DOCK_DOOR = gu.jdbcOracleConnection(
        f"""SELECT
            ...
            FROM DOCK_DOOR
            WHERE (TRUNC( DOCK_DOOR.CREATED_DTTM) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 1) OR (TRUNC( DOCK_DOOR.LAST_UPDATED_DTTM) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 1)""",
        username,
        password,
        connection_string,
    ...
    SQ_Shortcut_to_DOCK_DOOR_temp = SQ_Shortcut_to_DOCK_DOOR.toDF(
        *[
            "SQ_Shortcut_to_DOCK_DOOR___" + col
            for col in SQ_Shortcut_to_DOCK_DOOR.columns
        ]
    )
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_DOCK_DOOR_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
