...
UPD_VALIDATE = EXP_EVAL_VALUES_temp.selectExpr( \
	...
	.withColumn('pyspark_data_action', when((col('in_WM_EMP_DTL_ID').isNull()) ,(lit(0))) .otherwise(lit(1)))
...
Shortcut_to_WM_E_EMP_DTL = UPD_VALIDATE.selectExpr(
	...
	"CAST(EMP_DTL_ID AS INT) as WM_EMP_DTL_ID",
	"CAST(WHSE AS STRING) as WM_WHSE",
	"CAST(EMP_ID AS BIGINT) as WM_EMP_ID",
	"CAST(EFF_DATE_TIME AS TIMESTAMP) as EFF_TSTMP",
	"CAST(EMP_STAT_ID AS INT) as WM_EMP_STAT_ID",
	"CAST(SPVSR_EMP_ID AS BIGINT) as WM_SUPERVISOR_EMP_ID",
	"CAST(IS_SUPER AS TINYINT) as SUPERVISOR_FLAG",
	"CAST(ROLE_ID AS INT) as WM_ROLE_ID",
	"CAST(JOB_FUNC_ID AS INT) as WM_JOB_FUNC_ID",
	"CAST(DEPT_ID AS INT) as WM_DEPT_ID",
	"CAST(PAY_RATE AS DECIMAL(9,2)) as PAY_RATE",
	"CAST(PAY_SCALE_ID AS INT) as WM_PAY_SCALE_ID",
	...
	"CAST(DFLT_PERF_GOAL AS DECIMAL(5,2)) as DFLT_PERF_GOAL",
	"CAST(EXCLUDE_AUTO_CICO AS TINYINT) as EXCLUDE_AUTO_CICO_FLAG",
