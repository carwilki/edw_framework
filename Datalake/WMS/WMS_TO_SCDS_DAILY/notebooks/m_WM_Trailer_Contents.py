...
# Soft_Delete_Logic_trailer_contents1= ' $$Soft_Delete_Logic_trailer_contents1=update wm_trailer_contents set delete_flag=1, update_tstmp=current_timestamp where (location_id, WM_TRAILER_CONTENTS_ID) in (select location_id, WM_TRAILER_CONTENTS_ID from wm_trailer_contents log_table where log_table.DELETE_FLAG=0 and (cast(log_table.WM_CREATED_TSTMP as date) >= (cast('$$Prev_Run_Dt' as date)-14) OR cast(log_table.WM_LAST_UPDATED_TSTMP AS DATE) >= (cast('$$Prev_Run_Dt' AS DATE) - 14)) and (location_id, WM_TRAILER_CONTENTS_ID) not in (select   SP.location_id, log_pre.TRAILER_CONTENTS_ID from  scds_stg_prd..WM_TRAILER_CONTENTS_PRE log_pre,EDW_PRD..site_profile SP where SP.store_nbr =log_pre.dc_nbr));
# ' # args.soft_Delete_Logic_trailer_contents1

# update wm_trailer_contents set delete_flag=1, update_tstmp=current_timestamp where
Soft_Delete_Logic_trailer_contents1= f""" 
select  location_id, WM_TRAILER_CONTENTS_ID from {refined_perf_table}  
where  (location_id, WM_TRAILER_CONTENTS_ID) in 
(select location_id, WM_TRAILER_CONTENTS_ID 
from {refined_perf_table} log_table 
where log_table.DELETE_FLAG=0 
and (cast(log_table.WM_CREATED_TSTMP as date) >= (cast('{Prev_Run_Dt}' as date)-14) 
OR cast(log_table.WM_LAST_UPDATED_TSTMP AS DATE) >= (cast('{Prev_Run_Dt}' AS DATE) - 14)) 
and (location_id, WM_TRAILER_CONTENTS_ID) not in 
(select   SP.location_id, log_pre.TRAILER_CONTENTS_ID 
from  {raw_perf_table} log_pre,{site_profile_table} SP 
where SP.store_nbr =log_pre.dc_nbr))
"""

sd_df = spark.sql(Soft_Delete_Logic_trailer_contents1)

sd_df.createOrReplaceTempView('WM_TRAILER_CONTENTS_SD')

spark.sql(f"""
          MERGE INTO {refined_perf_table} tgt
          USING  WM_TRAILER_CONTENTS_SD src
          ON src.location_id = tgt.location_id and src.WM_TRAILER_CONTENTS_ID = tgt.WM_TRAILER_CONTENTS_ID
