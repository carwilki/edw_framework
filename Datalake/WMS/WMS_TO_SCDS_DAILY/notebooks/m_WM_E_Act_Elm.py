11ode converted on 2023-06-26 10:02:00
import os
import argparse
...
EXP_INT_CONV = SQ_Shortcut_to_WM_E_ACT_ELM_PRE_temp.selectExpr( \
	...
	"cast(SQ_Shortcut_to_WM_E_ACT_ELM_PRE___DC_NBR as int) as DC_NBR", \
...
FIL_NO_CHANGE_REC = JNR_WM_E_ACT_ELM_temp.selectExpr( \
    ...
    .filter("( ACT_ID IS NULL OR ELM_ID IS NULL ) OR ( in_WM_ACT_ID IS NULL OR in_WM_ELM_ID IS NULL ) OR ( ( in_WM_ACT_ID IS NOT NULL OR  in_WM_ELM_ID IS NOT NULL ) \
...
RTR_INS_UPD_DEL_DEL = EXP_EVALUATE.selectExpr( \
	...
	"THRUPUT_MSRMNT as THRUPUT_MSRMNT", \
	"SEQ_NBR as SEQ_NBR", \
	...
	"AVG_ACT_ID as AVG_ACT_ID", \
	"AVG_BY as AVG_BY", \
	"in_LOCATION_ID as in_LOCATION_ID", \
	"in_WM_ACT_ID as in_WM_ACT_ID", \
	"in_WM_ELM_ID as in_WM_ELM_ID", \
	"in_TIME_ALLOW as in_TIME_ALLOW", \
	"in_WM_THRUPUT_MSRMNT as in_WM_THRUPUT_MSRMNT", \
	"in_WM_SEQ_NBR as in_WM_SEQ_NBR", \
	"in_WM_CREATE_TSTMP as in_WM_CREATE_TSTMP", \
	"in_WM_MOD_TSTMP as in_WM_MOD_TSTMP", \
	"in_WM_USER_ID as in_WM_USER_ID", \
	"in_MISC_TXT_11 as in_MISC_TXT_11", \
	"in_MISC_TXT_21 as in_MISC_TXT_21", \
	"in_MISC_NUM_11 as in_MISC_NUM_11", \
	"in_MISC_NUM_21 as in_MISC_NUM_21", \
	"in_WM_VERSION_ID as in_WM_VERSION_ID", \
	"in_WM_AVG_ACT_ID as in_WM_AVG_ACT_ID", \
	"in_WM_AVG_BY as in_WM_AVG_BY", \
...
Shortcut_to_WM_E_ACT_ELM1.createOrReplaceTempView('WM_E_ACT_ELM_DEL')

spark.sql(f"""
          MERGE INTO {refined_perf_table} trg
          USING WM_E_ACT_ELM_DEL src
          ON (src.LOCATION_ID = trg.LOCATION_ID AND src.WM_ACT_ID = trg.WM_ACT_ID AND src.WM_ELM_ID = trg.WM_ELM_ID)
