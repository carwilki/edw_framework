...
def m_WM_Task_Dtl_PRE(dcnbr, env):
    ...
    query = f"""SELECT
                ...
                FROM TASK_DTL
                WHERE {Initial_Load} (TRUNC(TASK_DTL.CREATE_DATE_TIME) >= TRUNC(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 1) OR ( TRUNC(TASK_DTL.MOD_DATE_TIME)>= TRUNC(to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 1)"""
    

    SQ_Shortcut_to_TASK_DTL = gu.jdbcOracleConnection(query, username, password, connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    ...
    Shortcut_to_WM_TASK_DTL_PRE = EXP_TRN.selectExpr( 
    	...
    	"CAST(TASK_DTL_ID AS BIGINT) as TASK_DTL_ID", 
    	"CAST(TASK_ID AS BIGINT) as TASK_ID", 
    	"CAST(TASK_SEQ_NBR AS BIGINT) as TASK_SEQ_NBR", 
    	...
    	"CAST(ORIG_REQMT AS BIGINT) as ORIG_REQMT", 
    	"CAST(QTY_ALLOC AS BIGINT) as QTY_ALLOC", 
    	"CAST(QTY_PULLD AS BIGINT) as QTY_PULLD", 
    	"CAST(DEST_LOCN_ID AS STRING) as DEST_LOCN_ID", 
    	"CAST(DEST_LOCN_SEQ AS BIGINT) as DEST_LOCN_SEQ", 
    	...
    	"CAST(TASK_LOCN_SEQ AS BIGINT) as TASK_LOCN_SEQ", 
    	...
    	"CAST(PKT_SEQ_NBR AS BIGINT) as PKT_SEQ_NBR", 
    	"CAST(CARTON_NBR AS STRING) as CARTON_NBR", 
    	"CAST(CARTON_SEQ_NBR AS BIGINT) as CARTON_SEQ_NBR", 
    	"CAST(PIKR_NBR AS BIGINT) as PIKR_NBR", 
    	"CAST(TASK_CMPL_REF_NBR_SEQ AS BIGINT) as TASK_CMPL_REF_NBR_SEQ", 
    	"CAST(ALLOC_INVN_DTL_ID AS BIGINT) as ALLOC_INVN_DTL_ID", 
    	"CAST(SUBSTITUTION_FLAG AS BIGINT) as SUBSTITUTION_FLAG", 
    	"CAST(NEXT_TASK_ID AS BIGINT) as NEXT_TASK_ID", 
    	"CAST(NEXT_TASK_SEQ_NBR AS BIGINT) as NEXT_TASK_SEQ_NBR", 
    	...
    	"CAST(VOCOLLECT_POSN AS BIGINT) as VOCOLLECT_POSN", 
    	"CAST(VOCOLLECT_SHORT_FLAG AS STRING) as VOCOLLECT_SHORT_FLAG", 
    	"CAST(PAGE_NBR AS BIGINT) as PAGE_NBR", 
    	"CAST(RSN_CODE AS STRING) as RSN_CODE", 
    	"CAST(SLOT_NBR AS BIGINT) as SLOT_NBR", 
    	"CAST(SUBSLOT_NBR AS BIGINT) as SUBSLOT_NBR", 
    	...
    	"CAST(TRANS_INVN_TYPE AS BIGINT) as TRANS_INVN_TYPE", 
    ...
    gu.overwriteDeltaPartition(Shortcut_to_WM_TASK_DTL_PRE, "DC_NBR", dcnbr, target_table_name)
