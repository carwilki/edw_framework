...
def m_Yard_Zone_Slot_PRE(dcnbr, env):
    ...
    refine_table_name = "WM_YARD_ZONE_SLOT"
    ...
    SQ_Shortcut_to_YARD_ZONE_SLOT = gu.jdbcOracleConnection(
        query, username, password, connection_string
    ).withColumn("sys_row_id", monotonically_increasing_id())
    logger.info(
        "SQL query for SQ_Shortcut_to_YARD_ZONE_SLOT is executed and data is loaded using jdbc"
    )
    ...
    SQ_Shortcut_to_YARD_ZONE_SLOT_temp = SQ_Shortcut_to_YARD_ZONE_SLOT.toDF(
        *[
            "SQ_Shortcut_to_YARD_ZONE_SLOT___" + col
            for col in SQ_Shortcut_to_YARD_ZONE_SLOT.columns
        ]
    )
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_YARD_ZONE_SLOT_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
        "Shortcut_to_WM_YARD_ZONE_SLOT_PRE is written to the target table - "
        + target_table_name
    )
