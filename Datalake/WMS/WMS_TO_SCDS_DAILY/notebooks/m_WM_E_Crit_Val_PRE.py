...
def m_WM_E_Crit_Val_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_E_CRIT_VAL = gu.jdbcOracleConnection(
        f"""SELECT
    ...
    SQ_Shortcut_to_E_CRIT_VAL_temp = SQ_Shortcut_to_E_CRIT_VAL.toDF(
        *[
            "SQ_Shortcut_to_E_CRIT_VAL___" + col
            for col in SQ_Shortcut_to_E_CRIT_VAL.columns
        ]
    )
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_E_CRIT_VAL_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
