...
def m_WM_E_Msrmnt_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_E_MSRMNT = gu.jdbcOracleConnection(
        f"""SELECT
            E_MSRMNT.MSRMNT_ID,
        ...
        FROM E_MSRMNT
        WHERE (TRUNC( E_MSRMNT.CREATED_DTTM) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1) OR (TRUNC( E_MSRMNT.LAST_UPDATED_DTTM) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-1)""",
        username,
        password,
        connection_string,
    ...
    SQ_Shortcut_to_E_MSRMNT_temp = SQ_Shortcut_to_E_MSRMNT.toDF(
        *["SQ_Shortcut_to_E_MSRMNT___" + col for col in SQ_Shortcut_to_E_MSRMNT.columns]
    )
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_E_MSRMNT_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
