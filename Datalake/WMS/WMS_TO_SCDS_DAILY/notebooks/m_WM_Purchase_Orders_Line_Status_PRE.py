...
def m_WM_Purchase_Orders_Line_Status_PRE(dcnbr, env):
    ...
    refine_table_name = "WM_PURCHASE_ORDERS_LINE_STATUS"
    ...
    query = f"""SELECT
                ...
                FROM PURCHASE_ORDERS_LINE_STATUS"""
                        

    SQ_Shortcut_to_PURCHASE_ORDERS_LINE_STATUS = gu.jdbcOracleConnection(query, username, password, connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    ...
    gu.overwriteDeltaPartition(Shortcut_to_WM_PURCHASE_ORDERS_LINE_STATUS_PRE, "DC_NBR", dcnbr, target_table_name)
