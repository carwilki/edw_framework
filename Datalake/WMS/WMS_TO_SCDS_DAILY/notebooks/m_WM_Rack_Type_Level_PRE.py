...
def m_WM_Rack_Type_Level_PRE(dcnbr, env):
    ...
    spark.read.format('jdbc').option('url', connection_string).option( = jdbcOracleConnection(query, username, password, connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    ...
    SQ_Shortcut_to_RACK_TYPE_LEVEL_temp = SQ_Shortcut_to_RACK_TYPE_LEVEL.toDF(*["SQ_Shortcut_to_RACK_TYPE_LEVEL___" + col for col in SQ_Shortcut_to_RACK_TYPE_LEVEL.columns])
    ...
    Shortcut_to_WM_RACK_TYPE_LEVEL_PRE = EXPTRANS.selectExpr(
        ...
        "CAST(LEVEL_WT_LIMIT AS DECIMAL(13,4)) as LEVEL_WT_LIMIT",
        "CAST(LEVEL_HT AS DECIMAL(13,4)) as LEVEL_HT",
        "CAST(LEVEL_WIDTH AS DECIMAL(13,4)) as LEVEL_WIDTH",
        "CAST(LEVEL_DEPTH AS DECIMAL(13,4)) as LEVEL_DEPTH",
        "CAST(NBR_OF_SLOTS AS BIGINT) as NBR_OF_SLOTS",
        "CAST(SLOT_WT_LIMIT AS DECIMAL(13,4)) as SLOT_WT_LIMIT",
        "CAST(MAX_LANE_WT AS DECIMAL(13,4)) as MAX_LANE_WT",
        "CAST(HT_CLEARANCE AS DECIMAL(13,4)) as HT_CLEARANCE",
        "CAST(SIDE_CLEAR AS DECIMAL(13,4)) as SIDE_CLEAR",
        "CAST(MAX_STACKING AS BIGINT) as MAX_STACKING",
        "CAST(MAX_LANES AS BIGINT) as MAX_LANES",
        "CAST(GUIDE_WIDTH AS DECIMAL(13,4)) as GUIDE_WIDTH",
        "CAST(LANE_GUIDES AS TINYINT) as LANE_GUIDES",
        "CAST(SLOT_GUIDES AS TINYINT) as SLOT_GUIDES",
        "CAST(MOVE_GUIDES AS TINYINT) as MOVE_GUIDES",
        "CAST(USE_FULL_WID AS TINYINT) as USE_FULL_WID",
        "CAST(SHELF_THICKNESS AS DECIMAL(9,4)) as SHELF_THICKNESS",
        "CAST(LABEL_POS AS DECIMAL(9,4)) as LABEL_POS",
        "CAST(MAX_PICK_HT AS DECIMAL(9,4)) as MAX_PICK_HT",
        "CAST(SLOT_UNITS AS BIGINT) as SLOT_UNITS",
        "CAST(SHIP_UNITS AS BIGINT) as SHIP_UNITS",
        "CAST(MAX_SL_WID AS DECIMAL(13,4)) as MAX_SL_WID",
        "CAST(ONE_FACING_PER_DSW AS TINYINT) as ONE_FACING_PER_DSW",
        "CAST(LEVEL_PRIORITY AS BIGINT) as LEVEL_PRIORITY",
        "CAST(COST_TO_PICK AS DECIMAL(13,4)) as COST_TO_PICK",
        ...
        "CAST(USE_INCREMENT AS TINYINT) as USE_INCREMENT",
        "CAST(WIDTH_INCREMENT AS DECIMAL(13,4)) as WIDTH_INCREMENT",
        "CAST(MIN_SL_WIDTH AS DECIMAL(13,4)) as MIN_SL_WIDTH",
        "CAST(MIN_ITEM_WIDTH AS DECIMAL(13,4)) as MIN_ITEM_WIDTH",
        ...
        "CAST(USE_LEV_MAP_STR AS TINYINT) as USE_LEV_MAP_STR",
        "CAST(USE_POS_MAP_STR AS TINYINT) as USE_POS_MAP_STR",
        "CAST(USE_VERT_DIV AS TINYINT) as USE_VERT_DIV",
        "CAST(VERT_DIV_HT AS DECIMAL(13,4)) as VERT_DIV_HT",
        "CAST(MIN_CHANNEL_WID AS DECIMAL(13,4)) as MIN_CHANNEL_WID",
        "CAST(MAX_CHANNEL_WID AS DECIMAL(13,4)) as MAX_CHANNEL_WID",
        "CAST(MAX_CHANNEL_WT AS DECIMAL(13,4)) as MAX_CHANNEL_WT",
        "CAST(MAX_ADJ_SLOTS AS BIGINT) as MAX_ADJ_SLOTS",
        "CAST(MAX_EJECTOR_WT AS DECIMAL(13,4)) as MAX_EJECTOR_WT",
        "CAST(AFRAME_ROTATE AS TINYINT) as AFRAME_ROTATE",
        "CAST(AFRAME_MIN_HT AS DECIMAL(13,4)) as AFRAME_MIN_HT",
        "CAST(AFRAME_MAX_HT AS DECIMAL(13,4)) as AFRAME_MAX_HT",
        "CAST(AFRAME_MIN_LEN AS DECIMAL(13,4)) as AFRAME_MIN_LEN",
        "CAST(AFRAME_MAX_LEN AS DECIMAL(13,4)) as AFRAME_MAX_LEN",
        "CAST(AFRAME_MIN_WID AS DECIMAL(13,4)) as AFRAME_MIN_WID",
        "CAST(AFRAME_MAX_WID AS DECIMAL(13,4)) as AFRAME_MAX_WID",
        "CAST(AFRAME_MIN_WT AS DECIMAL(13,4)) as AFRAME_MIN_WT",
        "CAST(AFRAME_MAX_WT AS DECIMAL(13,4)) as AFRAME_MAX_WT",
        "CAST(REACH_DIST AS DECIMAL(13,4)) as REACH_DIST",
        "CAST(MOVE_GUIDES_RGT AS TINYINT) as MOVE_GUIDES_RGT",
        "CAST(MOVE_GUIDES_LFT AS TINYINT) as MOVE_GUIDES_LFT",
        "CAST(LOAD_TSTMP AS TIMESTAMP) as LOAD_TSTMP"
    )    
    overwriteDeltaPartition(Shortcut_to_WM_RACK_TYPE_LEVEL_PRE, "DC_NBR", dcnbr, target_table_name)
    logger.info("Shortcut_to_WM_RACK_TYPE_LEVEL_PRE is written to the target table - " + target_table_name)
