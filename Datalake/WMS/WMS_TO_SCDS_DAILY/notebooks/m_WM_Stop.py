...
(
   IF( TC_COMPANY_ID IS NULL, 0TC_COMPANY_ID ) != IF( i_WM_TC_COMPANY_ID IS NULL, 0i_WM_TC_COMPANY_ID )
OR IF( STOP_LOCATION_NAME IS NULL, ''STOP_LOCATION_NAME ) != IF( i_STOP_LOCATION_NAME IS NULL, ''i_STOP_LOCATION_NAME )
OR IF( FACILITY_ID IS NULL, 0FACILITY_ID ) != IF( i_WM_FACILITY_ID IS NULL, 0i_WM_FACILITY_ID )
...
OR IF( WAVE_ID IS NULL, 0WAVE_ID ) != IF( i_WM_WAVE_ID IS NULL, 0i_WM_WAVE_ID )
OR IF( WAVE_OPTION_ID IS NULL, 0WAVE_OPTION_ID ) != IF( i_WM_WAVE_OPTION_ID IS NULL, 0i_WM_WAVE_OPTION_ID )
OR IF( WAVE_OPTION_LIST_ID IS NULL, 0WAVE_OPTION_LIST_ID ) != IF( i_WM_WAVE_OPTION_LIST_ID IS NULL, 0i_WM_WAVE_OPTION_LIST_ID )
OR IF( DOCK_DISPATCH_CAP_ID IS NULL, 0DOCK_DISPATCH_CAP_ID ) != IF( i_WM_DOCK_DISPATCH_CAP_ID IS NULL, 0i_WM_DOCK_DISPATCH_CAP_ID )
OR IF( SEGMENT_ID IS NULL, 0SEGMENT_ID ) != IF( i_WM_SEGMENT_ID IS NULL, 0i_WM_SEGMENT_ID )
OR IF( SEGMENT_STOP_SEQ IS NULL, 0SEGMENT_STOP_SEQ ) != IF( i_SEGMENT_STOP_SEQ IS NULL, 0i_SEGMENT_STOP_SEQ )
OR IF( HANDLER IS NULL, 0HANDLER ) != IF( i_HANDLER IS NULL, 0i_HANDLER )
OR IF( REPORTED_HANDLER IS NULL, 0REPORTED_HANDLER ) != IF( i_REPORTED_HANDLER IS NULL, 0i_REPORTED_HANDLER )
OR IF( DELAY_TYPE IS NULL, 0DELAY_TYPE ) != IF( i_DELAY_TYPE IS NULL, 0i_DELAY_TYPE )
OR IF( STOP_STATUS IS NULL, 0STOP_STATUS ) != IF( i_STOP_STATUS IS NULL, 0i_STOP_STATUS )
...
OR IF( BOL_TYPE IS NULL, 0BOL_TYPE ) != IF( i_BOL_TYPE IS NULL, 0i_BOL_TYPE )
...
OR IF( ORDER_WINDOW_VIOLATION IS NULL, 0ORDER_WINDOW_VIOLATION ) != IF( i_ORDER_WINDOW_VIOLATION IS NULL, 0i_ORDER_WINDOW_VIOLATION )
OR IF( DOCK_VIOLATION IS NULL, 0DOCK_VIOLATION ) != IF( i_DOCK_VIOLATION IS NULL, 0i_DOCK_VIOLATION )
OR IF( WAVE_OPTION_VIOLATION IS NULL, 0WAVE_OPTION_VIOLATION ) != IF( i_WAVE_OPTION_VIOLATION IS NULL, 0i_WAVE_OPTION_VIOLATION )
OR IF( AUTO_APPOINTMENT IS NULL, 0AUTO_APPOINTMENT ) != IF( AUTO_APPOINTMENT_FLAG IS NULL, 0AUTO_APPOINTMENT_FLAG )
OR IF( IS_APPT_REQD IS NULL, 0IS_APPT_REQD ) != IF( APPT_REQD_FLAG IS NULL, 0APPT_REQD_FLAG )
OR IF( IS_ADDR_OVERRIDE IS NULL, 0IS_ADDR_OVERRIDE ) != IF( ADDR_OVERRIDE_FLAG IS NULL, 0ADDR_OVERRIDE_FLAG )
OR IF( LOAD_CLOSED_INDICATOR IS NULL, 0LOAD_CLOSED_INDICATOR ) != IF( LOAD_CLOSED_FLAG IS NULL, 0LOAD_CLOSED_FLAG )
OR IF( IS_WAVE_MAN_CHANGED IS NULL, 0IS_WAVE_MAN_CHANGED ) != IF( WAVE_MAN_CHANGED_FLAG IS NULL, 0WAVE_MAN_CHANGED_FLAG )
OR IF( ON_STATIC_ROUTE IS NULL, 0ON_STATIC_ROUTE ) != IF( ON_STATIC_ROUTE_FLAG IS NULL, 0ON_STATIC_ROUTE_FLAG )
OR IF( HAS_NOTES IS NULL, 0HAS_NOTES ) != IF( HAS_NOTES_FLAG IS NULL, 0HAS_NOTES_FLAG )
OR IF( IS_RECONSIGNED IS NULL, 0IS_RECONSIGNED ) != IF( RECONSIGNED_FLAG IS NULL, 0RECONSIGNED_FLAG )
OR IF( IS_EST_ARR_USER_DEFINED IS NULL, 0IS_EST_ARR_USER_DEFINED ) != IF( EST_ARR_USER_DEFINED_FLAG IS NULL, 0EST_ARR_USER_DEFINED_FLAG )
OR IF( IS_EST_DEP_USER_DEFINED IS NULL, 0IS_EST_DEP_USER_DEFINED ) != IF( EST_DEP_USER_DEFINED_FLAG IS NULL, 0EST_DEP_USER_DEFINED_FLAG )
OR IF( DROP_HOOK_INDICATOR IS NULL, 0DROP_HOOK_INDICATOR ) != IF( DROP_HOOK_FLAG IS NULL, 0DROP_HOOK_FLAG )
...
OR IF( APPOINTMENT_DURATION IS NULL, 0APPOINTMENT_DURATION ) != IF( i_APPOINTMENT_DURATION IS NULL, 0i_APPOINTMENT_DURATION )
...
OR IF( HANDLING_TIME IS NULL, 0HANDLING_TIME ) != IF( i_HANDLING_TIME IS NULL, 0i_HANDLING_TIME )
OR IF( BREAK_TIME IS NULL, 0BREAK_TIME ) != IF( i_BREAK_TIME IS NULL, 0i_BREAK_TIME )
OR IF( WAITING_TIME IS NULL, 0WAITING_TIME ) != IF( i_WAITING_TIME IS NULL, 0i_WAITING_TIME )
OR IF( HITCH_TIME IS NULL, 0HITCH_TIME ) != IF( i_HITCH_TIME IS NULL, 0i_HITCH_TIME )
OR IF( DRIVE_TIME IS NULL, 0DRIVE_TIME ) != IF( i_DRIVE_TIME IS NULL, 0i_DRIVE_TIME )
OR IF( STOP_TIME_MODIFIED IS NULL, 0STOP_TIME_MODIFIED ) != IF( i_STOP_TIME_MODIFIED IS NULL, 0i_STOP_TIME_MODIFIED )
OR IF( DISTANCE IS NULL, 0DISTANCE ) != IF( i_DISTANCE IS NULL, 0i_DISTANCE )
...
OR IF( STOP_TZ IS NULL, 0STOP_TZ ) != IF( i_WM_STOP_TZ IS NULL, 0i_WM_STOP_TZ )
OR IF( DEGREE_LATE_EARLY IS NULL, 0DEGREE_LATE_EARLY ) != IF( i_DEGREE_LATE_EARLY IS NULL, 0i_DEGREE_LATE_EARLY )
OR IF( ARRIVAL_LATE_REASON_ID IS NULL, 0ARRIVAL_LATE_REASON_ID ) != IF( i_WM_ARRIVAL_LATE_REASON_ID IS NULL, 0i_WM_ARRIVAL_LATE_REASON_ID )
OR IF( DEPARTURE_LATE_REASON_ID IS NULL, 0DEPARTURE_LATE_REASON_ID ) != IF( i_WM_DEPARTURE_LATE_REASON_ID IS NULL, 0i_WM_DEPARTURE_LATE_REASON_ID )
OR IF( STOP_REVENUE IS NULL, 0STOP_REVENUE ) != IF( i_STOP_REVENUE IS NULL, 0i_STOP_REVENUE )
...
OR IF( STOP_MARGIN IS NULL, 0STOP_MARGIN ) != IF( i_STOP_MARGIN IS NULL, 0i_STOP_MARGIN )
...
OR IF( STOP_COST IS NULL, 0STOP_COST ) != IF( i_STOP_COST IS NULL, 0i_STOP_COST )
OR IF( STOP_COST_CURRENCY_CODE IS NULL, ''STOP_COST_CURRENCY_CODE ) != IF( i_STOP_COST_CURRENCY_CD IS NULL, ''i_STOP_COST_CURRENCY_CD )
OR IF( STOP_LINEHAUL_REVENUE IS NULL, 0STOP_LINEHAUL_REVENUE ) != IF( i_STOP_LINEHAUL_REVENUE IS NULL, 0i_STOP_LINEHAUL_REVENUE )
OR IF( ACCESSORIAL_REVENUE IS NULL, 0ACCESSORIAL_REVENUE ) != IF( i_ACCESSORIAL_REVENUE IS NULL, 0i_ACCESSORIAL_REVENUE )
OR IF( STOPOFF_REVENUE IS NULL, 0STOPOFF_REVENUE ) != IF( i_STOP_OFF_REVENUE IS NULL, 0i_STOP_OFF_REVENUE )
OR IF( CM_DISCOUNT_REVENUE IS NULL, 0CM_DISCOUNT_REVENUE ) != IF( i_CM_DISCOUNT_REVENUE IS NULL, 0i_CM_DISCOUNT_REVENUE )
OR IF( CAPACITY IS NULL, 0CAPACITY ) != IF( i_CAPACITY IS NULL, 0i_CAPACITY )
OR IF( SIZE_UOM_ID IS NULL, 0SIZE_UOM_ID ) != IF( i_WM_SIZE_UOM_ID IS NULL, 0i_WM_SIZE_UOM_ID )
...
OR IF( HIBERNATE_VERSION IS NULL, 0HIBERNATE_VERSION ) != IF( i_WM_HIBERNATE_VERSION IS NULL, 0i_WM_HIBERNATE_VERSION ) 
)
"""
...
Shortcut_to_WM_STOP1 = UPD_INS_UPD.selectExpr( \
	...
	"CAST(STOP_SEQ AS BIGINT) as WM_STOP_SEQ", \
	...
	"CAST(WAVE_OPTION_LIST_ID AS BIGINT) as WM_WAVE_OPTION_LIST_ID", \
	"CAST(DOCK_DISPATCH_CAP_ID AS BIGINT) as WM_DOCK_DISPATCH_CAP_ID", \
	"CAST(SEGMENT_ID AS BIGINT) as WM_SEGMENT_ID", \
	"CAST(SEGMENT_STOP_SEQ AS BIGINT) as SEGMENT_STOP_SEQ", \
	"CAST(HANDLER AS BIGINT) as HANDLER", \
	"CAST(REPORTED_HANDLER AS BIGINT) as REPORTED_HANDLER", \
	...
	"CAST(BOL_TYPE AS BIGINT) as BOL_TYPE", \
	...
	"CAST(IS_APPT_REQD AS BIGINT) as APPT_REQD_FLAG", \
	"CAST(IS_ADDR_OVERRIDE AS BIGINT) as ADDR_OVERRIDE_FLAG", \
	"CAST(LOAD_CLOSED_INDICATOR AS BIGINT) as LOAD_CLOSED_FLAG", \
	"CAST(IS_WAVE_MAN_CHANGED AS BIGINT) as WAVE_MAN_CHANGED_FLAG", \
	"CAST(ON_STATIC_ROUTE AS BIGINT) as ON_STATIC_ROUTE_FLAG", \
	"CAST(HAS_NOTES AS BIGINT) as HAS_NOTES_FLAG", \
	"CAST(IS_RECONSIGNED AS BIGINT) as RECONSIGNED_FLAG", \
	"CAST(IS_EST_ARR_USER_DEFINED AS BIGINT) as EST_ARR_USER_DEFINED_FLAG", \
	"CAST(IS_EST_DEP_USER_DEFINED AS BIGINT) as EST_DEP_USER_DEFINED_FLAG", \
	"CAST(DROP_HOOK_INDICATOR AS BIGINT) as DROP_HOOK_FLAG", \
	...
	"CAST(HANDLING_TIME AS BIGINT) as HANDLING_TIME", \
	"CAST(BREAK_TIME AS BIGINT) as BREAK_TIME", \
	"CAST(WAITING_TIME AS BIGINT) as WAITING_TIME", \
	"CAST(HITCH_TIME AS BIGINT) as HITCH_TIME", \
	"CAST(DRIVE_TIME AS BIGINT) as DRIVE_TIME", \
	"CAST(STOP_TIME_MODIFIED AS BIGINT) as STOP_TIME_MODIFIED", \
	...
	"CAST(STOP_TZ AS BIGINT) as WM_STOP_TZ", \
	"CAST(DEGREE_LATE_EARLY AS BIGINT) as DEGREE_LATE_EARLY", \
	"CAST(ARRIVAL_LATE_REASON_ID AS BIGINT) as WM_ARRIVAL_LATE_REASON_ID", \
	"CAST(DEPARTURE_LATE_REASON_ID AS BIGINT) as WM_DEPARTURE_LATE_REASON_ID", \
	"CAST(STOP_REVENUE AS BIGINT) as STOP_REVENUE", \
	"CAST(STOP_REVENUE_CURRENCY_CODE AS STRING) as STOP_REVENUE_CURRENCY_CD", \
	"CAST(STOP_MARGIN AS BIGINT) as STOP_MARGIN", \
	...
	"CAST(STOP_LINEHAUL_REVENUE AS BIGINT) as STOP_LINEHAUL_REVENUE", \
	"CAST(ACCESSORIAL_REVENUE AS BIGINT) as ACCESSORIAL_REVENUE", \
	"CAST(STOPOFF_REVENUE AS BIGINT) as STOP_OFF_REVENUE", \
...
Shortcut_to_WM_STOP1.write.saveAsTable(f'{raw}.WM_STOP', mode = 'append')
Shortcut_to_WM_STOP1 = UPD_INS_UPD.selectExpr( \
	"CAST(LOCATION_ID AS BIGINT) as LOCATION_ID", \
	"CAST(SHIPMENT_ID AS BIGINT) as WM_SHIPMENT_ID", \
	"CAST(STOP_SEQ AS BIGINT) as WM_STOP_SEQ", \
