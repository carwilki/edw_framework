...
def m_WM_UN_Number_PRE(dcnbr, env):
    ...
    refine_table_name = "WM_UN_NUMBER"
    ...
    SQ_Shortcut_to_UN_NUMBER = gu.jdbcOracleConnection(query, username, password, connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    ...
    Shortcut_to_WM_UN_NUMBER_PRE = EXPTRANS.selectExpr( 
    	...
    	"CAST(HAZMAT_UOM AS BIGINT) as HAZMAT_UOM", 
    	"CAST(MAX_HAZMAT_QTY AS BIGINT) as MAX_HAZMAT_QTY", 
    	"CAST(REGULATION_SET AS STRING) as REGULATION_SET", 
    	"CAST(TRANS_MODE AS STRING) as TRANS_MODE", 
    	"CAST(REPORTABLE_QTY AS BIGINT) as REPORTABLE_QTY", 
    	...
    	"CAST(RADIOACTIVE_QTY_CALC_FACTOR AS BIGINT) as RADIOACTIVE_QTY_CALC_FACTOR", 
    	...
    	"CAST(NET_EXPLOSIVE_QUANTITY_FACTOR AS BIGINT) as NET_EXPLOSIVE_QUANTITY_FACTOR", 
    ...
    gu.overwriteDeltaPartition(Shortcut_to_WM_UN_NUMBER_PRE, "DC_NBR", dcnbr, target_table_name)
