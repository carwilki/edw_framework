...
def m_WM_C_Leader_Audit_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_C_LEADER_AUDIT = gu.jdbcOracleConnection(
        f"""SELECT
    ...
    SQ_Shortcut_to_C_LEADER_AUDIT_temp = SQ_Shortcut_to_C_LEADER_AUDIT.toDF(
        *[
            "SQ_Shortcut_to_C_LEADER_AUDIT___" + col
            for col in SQ_Shortcut_to_C_LEADER_AUDIT.columns
        ]
    )
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_C_LEADER_AUDIT_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
