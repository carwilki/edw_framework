...
def m_WM_Asn_Detail_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_ASN_DETAIL = gu.jdbcOracleConnection(
        f"""SELECT
    ...
    SQ_Shortcut_to_ASN_DETAIL_temp = SQ_Shortcut_to_ASN_DETAIL.toDF(
        *[
            "SQ_Shortcut_to_ASN_DETAIL___" + col
            for col in SQ_Shortcut_to_ASN_DETAIL.columns
        ]
    )
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_ASN_DETAIL_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
