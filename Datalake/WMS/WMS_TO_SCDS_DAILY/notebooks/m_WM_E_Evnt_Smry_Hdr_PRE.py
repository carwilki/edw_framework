...
def m_WM_E_Evnt_Smry_Hdr_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_E_EVNT_SMRY_HDR = jdbcOracleConnection(
        f"""SELECT
            ...
            WHERE (TRUNC( E_EVNT_SMRY_HDR.CREATE_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (TRUNC( E_EVNT_SMRY_HDR.MOD_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14)""",username,password,connection_string).withColumn("sys_row_id", monotonically_increasing_id())
