...
def m_WM_E_Evnt_Smry_Hdr_PRE(dcnbr, env):
    ...
    refine_table_name = "E_EVNT_SMRY_HDR"
    ...
    SQ_Shortcut_to_E_EVNT_SMRY_HDR = gu.jdbcOracleConnection(
        f"""SELECT
            ...
            WHERE (TRUNC( E_EVNT_SMRY_HDR.CREATE_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (TRUNC( E_EVNT_SMRY_HDR.MOD_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14)""",
        username,
        password,
        connection_string,
    ...
    SQ_Shortcut_to_E_EVNT_SMRY_HDR_temp = SQ_Shortcut_to_E_EVNT_SMRY_HDR.toDF(
        *[
            "SQ_Shortcut_to_E_EVNT_SMRY_HDR___" + col
            for col in SQ_Shortcut_to_E_EVNT_SMRY_HDR.columns
        ]
    )
    ...
    Shortcut_to_WM_E_EVNT_SMRY_HDR_PRE = EXPTRANS.selectExpr(
        ...
        "CAST(STD_TIME_ALLOW AS BIGINT) as STD_TIME_ALLOW",
        "CAST(EMP_PERF_ALLOW AS BIGINT) as EMP_PERF_ALLOW",
        "CAST(PERF_ADJ_AMT AS BIGINT) as PERF_ADJ_AMT",
        "CAST(SCHED_ADJ_AMT AS BIGINT) as SCHED_ADJ_AMT",
        "CAST(SCHED_UNITS_QTY AS BIGINT) as SCHED_UNITS_QTY",
        "CAST(ACTL_TIME AS BIGINT) as ACTL_TIME",
        "CAST(ERROR_CNT AS BIGINT) as ERROR_CNT",
        ...
        "CAST(TMU_RECALC_COUNT AS BIGINT) as TMU_RECALC_COUNT",
        ...
        "CAST(TEAM_STD_SMRY_ID AS BIGINT) as TEAM_STD_SMRY_ID",
        ...
        "CAST(PROC_ZONE_ID AS BIGINT) as PROC_ZONE_ID",
        ...
        "CAST(TEAM_CHG_ID AS BIGINT) as TEAM_CHG_ID",
        "CAST(ACTUAL_END_DATE AS TIMESTAMP) as ACTUAL_END_DATE",
        "CAST(SPVSR_LOGIN_USER_ID AS STRING) as SPVSR_LOGIN_USER_ID",
        "CAST(PAID_BRK_OVERLAP AS BIGINT) as PAID_BRK_OVERLAP",
        "CAST(UNPAID_BRK_OVERLAP AS BIGINT) as UNPAID_BRK_OVERLAP",
        "CAST(EMP_PERF_SMRY_ID AS BIGINT) as EMP_PERF_SMRY_ID",
        "CAST(HDR_MSG_ID AS BIGINT) as HDR_MSG_ID",
        ...
        "CAST(UNIT_MIN AS BIGINT) as UNIT_MIN",
        "CAST(CONSOL_EVNT_TIME AS BIGINT) as CONSOL_EVNT_TIME",
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_E_EVNT_SMRY_HDR_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
