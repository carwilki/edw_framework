...
raw_perf_table = f"{raw}.WM_E_ELM_PRE"
refined_perf_table = f"{refine}.WM_E_ELM"
...
.withColumn("FIL_UNCHANGED_ROWS___v_CREATE_DATE_TIME", expr("""IF(FIL_UNCHANGED_ROWS___CREATE_DATE_TIME IS NULL, date'1900-01-01', FIL_UNCHANGED_ROWS___CREATE_DATE_TIME)""")) \
	.withColumn("FIL_UNCHANGED_ROWS___v_MOD_DATE_TIME", expr("""IF(FIL_UNCHANGED_ROWS___MOD_DATE_TIME IS NULL, date'1900-01-01', FIL_UNCHANGED_ROWS___MOD_DATE_TIME)""")) \
	.withColumn("FIL_UNCHANGED_ROWS___v_i_WM_CREATE_TSTMP", expr("""IF(FIL_UNCHANGED_ROWS___i_WM_CREATE_TSTMP IS NULL, date'1900-01-01', FIL_UNCHANGED_ROWS___i_WM_CREATE_TSTMP)""")) \
	.withColumn("FIL_UNCHANGED_ROWS___v_i_WM_MOD_TSTMP", expr("""IF(FIL_UNCHANGED_ROWS___i_WM_MOD_TSTMP IS NULL, date'1900-01-01', FIL_UNCHANGED_ROWS___i_WM_MOD_TSTMP)"""))
...
EXP_OUTPUT_VALIDATOR = FIL_UNCHANGED_ROWS_temp.selectExpr( \
	...
	"CASE WHEN  LTRIM ( RTRIM ( UPPER ( FIL_UNCHANGED_ROWS___CORE_FLAG ) ) ) IN ('Y','1') THEN '1' ELSE '0' END as o_CORE_FLAG", \
...
RTR_INS_UPD_DEL_DELETE = EXP_OUTPUT_VALIDATOR.selectExpr( \
	...
	"o_CORE_FLAG as CORE_FLAG", \
	"MSRMNT_ID as MSRMNT_ID", \
	"TIME_ALLOW as TIME_ALLOW", \
	"ELM_GRP_ID as ELM_GRP_ID", \
	...
	"UNQ_SEED_ID as UNQ_SEED_ID", \
	"SIM_WHSE as SIM_WHSE", \
	"ORIG_NAME as ORIG_NAME", \
	"i_LOCATION_ID1 as i_LOCATION_ID1", \
	"i_WM_ELM_ID as i_WM_ELM_ID", \
	"i_WM_ELM_NAME as i_WM_ELM_NAME", \
	"i_WM_ELM_DESC as i_WM_ELM_DESC", \
	"i_ORIG_NAME1 as i_ORIG_NAME1", \
	"i_CORE_FLAG1 as i_CORE_FLAG1", \
	"i_WM_MSRMNT_ID as i_WM_MSRMNT_ID", \
	"i_TIME_ALLOW1 as i_TIME_ALLOW1", \
	"i_WM_ELM_GRP_ID as i_WM_ELM_GRP_ID", \
	"i_WM_UNQ_SEED_ID as i_WM_UNQ_SEED_ID", \
	"i_WM_SIM_WHSE as i_WM_SIM_WHSE", \
...
Shortcut_to_WM_E_ELM1 = UPD_INS_UPD.selectExpr(
	"CAST(LOCATION_ID1 AS BIGINT) as LOCATION_ID",
	"CAST(ELM_ID1 AS INT) as WM_ELM_ID",
	"CAST(NAME1 AS STRING) as WM_ELM_NAME",
	"CAST(DESCRIPTION1 AS STRING) as WM_ELM_DESC",
	"CAST(ORIG_NAME1 AS STRING) as ORIG_NAME",
	"CAST(CORE_FLAG1 AS TINYINT) as CORE_FLAG",
	"CAST(MSRMNT_ID1 AS INT) as WM_MSRMNT_ID",
	"CAST(TIME_ALLOW1 AS DECIMAL(9,4)) as TIME_ALLOW",
	"CAST(ELM_GRP_ID1 AS INT) as WM_ELM_GRP_ID",
	"CAST(UNQ_SEED_ID1 AS INT) as WM_UNQ_SEED_ID",
	"CAST(SIM_WHSE1 AS STRING) as WM_SIM_WHSE",
...
Shortcut_to_WM_E_ELM11.createOrReplaceTempView('WM_E_ELM_DEL')

spark.sql(f"""
          MERGE INTO {refined_perf_table} trg
          USING WM_E_ELM_DEL src
          ON (src.LOCATION_ID = trg.LOCATION_ID AND src.WM_ELM_ID = trg.WM_ELM_ID )
