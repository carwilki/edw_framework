...
def m_WM_E_Shift_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_E_SHIFT = jdbcOracleConnection(
        f"""SELECT
            ...
            FROM E_SHIFT
            WHERE (TRUNC( CREATED_DTTM) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-14) OR (TRUNC( LAST_UPDATED_DTTM) >=  TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-14) OR (TRUNC( CREATE_DATE_TIME) >=  TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-14 )OR (TRUNC( MOD_DATE_TIME) >=  TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-14)""",username,password,connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    ...
    SQ_Shortcut_to_E_SHIFT_temp = SQ_Shortcut_to_E_SHIFT.toDF(*["SQ_Shortcut_to_E_SHIFT___" + col for col in SQ_Shortcut_to_E_SHIFT.columns])
    ...
    overwriteDeltaPartition(Shortcut_to_WM_E_SHIFT_PRE1,"DC_NBR",dcnbr,target_table_name)
    logger.info(
