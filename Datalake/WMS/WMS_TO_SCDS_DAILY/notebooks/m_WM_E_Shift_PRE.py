...
def m_WM_E_Shift_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_E_SHIFT = gu.jdbcOracleConnection(
        f"""SELECT
                E_SHIFT.SHIFT_ID,
                E_SHIFT.EFF_DATE,
                E_SHIFT.SHIFT_CODE,
            ...
            FROM E_SHIFT
            WHERE (TRUNC( CREATED_DTTM) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-14) OR (TRUNC( LAST_UPDATED_DTTM) >=  TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-14) OR (TRUNC( CREATE_DATE_TIME) >=  TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-14 )OR (TRUNC( MOD_DATE_TIME) >=  TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS'))-14)""",
        username,
        password,
        connection_string,
