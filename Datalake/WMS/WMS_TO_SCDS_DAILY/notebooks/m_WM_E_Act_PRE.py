...
def m_WM_E_Act_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_E_ACT = gu.jdbcOracleConnection(
        f"""SELECT
                E_ACT.ACT_ID,
                ...
                E_ACT.THRUPUT_GOAL
            FROM E_ACT
            WHERE (TRUNC( E_ACT.CREATE_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14 ) OR (TRUNC( E_ACT.MOD_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14 ) OR (TRUNC( E_ACT.CREATED_DTTM) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14) OR (TRUNC( E_ACT.LAST_UPDATED_DTTM) >= TRUNC( to_date('{Prev_Run_Dt}','MM/DD/YYYY HH24:MI:SS')) - 14)""",
        username,
        password,
        connection_string,
    ...
    SQ_Shortcut_to_E_ACT_temp = SQ_Shortcut_to_E_ACT.toDF(
        *["SQ_Shortcut_to_E_ACT___" + col for col in SQ_Shortcut_to_E_ACT.columns]
    )
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_E_ACT_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
        "Shortcut_to_WM_E_ACT_PRE is written to the target table - " + target_table_name
    )
