...
def m_WM_E_Act_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_E_ACT = jdbcOracleConnection(
        f"""SELECT
                ...
                E_ACT.DISPLAY_UOM,
                E_ACT.THRUPUT_GOAL
            FROM {source_schema}.E_ACT
            WHERE (TRUNC( E_ACT.CREATE_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','YYYY-MM-DD')) - 14 ) OR (TRUNC( E_ACT.MOD_DATE_TIME) >= TRUNC( to_date('{Prev_Run_Dt}','YYYY-MM-DD')) - 14 ) OR (TRUNC( E_ACT.CREATED_DTTM) >= TRUNC( to_date('{Prev_Run_Dt}','YYYY-MM-DD')) - 14) OR (TRUNC( E_ACT.LAST_UPDATED_DTTM) >= TRUNC( to_date('{Prev_Run_Dt}','YYYY-MM-DD')) - 14)""",username,password,connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    ...
    Shortcut_to_WM_E_ACT_PRE = EXPTRANS.selectExpr(
        ...
        "CAST(OVERRIDE_PROC_ZONE_ID AS INT) as OVERRIDE_PROC_ZONE_ID",
        "CAST(PROC_ZONE_LOCN_IND AS INT) as PROC_ZONE_LOCN_IND",
        ...
        "CAST(VHCL_TYPE_ID AS INT) as VHCL_TYPE_ID",
