...
def m_WM_Yard_Zone_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_YARD_ZONE = jdbcOracleConnection(query, username, password, connection_string).withColumn("sys_row_id", monotonically_increasing_id())
    logger.info("SQL query for SQ_Shortcut_to_YARD_ZONE is executed and data is loaded using jdbc")
    ...
    SQ_Shortcut_to_YARD_ZONE_temp = SQ_Shortcut_to_YARD_ZONE.toDF(*["SQ_Shortcut_to_YARD_ZONE___" + col for col in SQ_Shortcut_to_YARD_ZONE.columns])
    ...
    Shortcut_to_WM_YARD_ZONE_PRE = EXPTRANS.selectExpr(
        ...
        "CAST(PUTAWAY_ELIGIBLE AS TINYINT) as PUTAWAY_ELIGIBLE",
    ...
    overwriteDeltaPartition(Shortcut_to_WM_YARD_ZONE_PRE, "DC_NBR", dcnbr, target_table_name)
    logger.info("Shortcut_to_WM_YARD_ZONE_PRE is written to the target table - " + target_table_name)
