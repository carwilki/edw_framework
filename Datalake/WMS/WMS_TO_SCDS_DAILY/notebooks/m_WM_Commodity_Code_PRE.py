...
def m_WM_Commodity_Code_PRE(dcnbr, env):
    ...
    SQ_Shortcut_to_COMMODITY_CODE = gu.jdbcOracleConnection(
        f"""SELECT
    ...
    SQ_Shortcut_to_COMMODITY_CODE_temp = SQ_Shortcut_to_COMMODITY_CODE.toDF(
        *[
            "SQ_Shortcut_to_COMMODITY_CODE___" + col
            for col in SQ_Shortcut_to_COMMODITY_CODE.columns
        ]
    )
    ...
    gu.overwriteDeltaPartition(
        Shortcut_to_WM_COMMODITY_CODE_PRE, "DC_NBR", dcnbr, target_table_name
    )
    logger.info(
