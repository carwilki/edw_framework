...
,A.EMPLOYEE_ID
,B.LOCATION_ID
,SUM(HOURS_WORKED) OVER (PARTITION BY((
to_date('CURRENT_DATE') - (DATE_PART('DOW', 'CURRENT_DATE'  )-1)) - 7), B.LOCATION_ID, A.EMPLOYEE_ID) HOURS_WORKED
,SUM(HOURS_WORKED) OVER (PARTITION BY((
to_date('CURRENT_DATE') - (DATE_PART('DOW', 'CURRENT_DATE'  )-1)) - 7), A.EMPLOYEE_ID) TOT_HOURS_WORKED
FROM
 (SELECT *
  FROM   (SELECT *
          ...
          -- UNION
          -- SELECT *
          --   FROM {empl_protected}.legacy_EMPLOYEE_PROFILE_DAY_PSOFT /*  USE EMPLOYEE_PROFILE_DAY_PSOFT UNTIL 1/11/2011 */
          --   WHERE DAY_DT > CAST('20101128' AS TIMESTAMP)
        ) T /*  THIS WILL BE THE CONVERSION DATE. */
  WHERE  DAY_DT = ((
to_date('CURRENT_DATE') - (DATE_PART('DOW', 'CURRENT_DATE'  )-1)) - 7)) A

,(SELECT *
    FROM {empl_protected}.legacy_EMPL_EMPL_LOC_WK
  -- UNION
  -- SELECT *
  --   FROM {empl_protected}.legacy_EMPL_EMPL_LOC_WK_PSOFT
  ) B /*  USE EMPL_EMPL_LOC_WK_PSOFT UNTIL AFTER 4/15/2011, THEN IT CAN BE REMOVED. */
,{legacy}.EARNINGS_ID E

WHERE A.FULLPT_FLAG    IN ('F')
...
/* /* 13 week no change*/ */
  AND A.EMPL_HIRE_DT    < ((
to_date('CURRENT_DATE') - (DATE_PART('DOW', 'CURRENT_DATE')-1)) - 91 - 7)
...
to_date('CURRENT_DATE') - (DATE_PART('DOW', 'CURRENT_DATE')-1)) - 91 - 7)
  ...
  AND (A.LOCATION_CHG_DT < ((
to_date('CURRENT_DATE') - (DATE_PART('DOW', 'CURRENT_DATE')-1)) - 91 - 7) OR A.LOCATION_CHG_DT IS NULL)
  ...
  AND B.WEEK_DT         > ((
to_date('CURRENT_DATE') - (DATE_PART('DOW', 'CURRENT_DATE' )-1)) - 94 - 0)

  AND B.WEEK_DT         <= ((
to_date('CURRENT_DATE') - (DATE_PART('DOW', 'CURRENT_DATE' )-1)) - 7)
...
#   AND B.WEEK_DT         <= ((CURRENT_DATE - (DATE_PART('DOW', CURRENT_DATE )-1)) - 7)
# """).withColumn("sys_row_id", monotonically_increasing_id())
# # Conforming fields names to the component layout
# SQ_Shortcut_to_EMPLOYEE_PROFILE = SQ_Shortcut_to_EMPLOYEE_PROFILE \
# 	.withColumnRenamed(SQ_Shortcut_to_EMPLOYEE_PROFILE.columns[0],'PERIOD_DT') \
...
# 35_temp = 35.toDF(*["35___" + col for col in 35.columns])
...
EXPTRANS = SQ_Shortcut_to_EMPLOYEE_PROFILE_temp.withColumn("HOURS_WORKED1", col('SQ_Shortcut_to_EMPLOYEE_PROFILE___HOURS_WORKED') / 13) \
	...
	"IF (var_AVG_HOURS_WORKED > 35.99, 0, 1) as Flag"
)

# COMMAND ----------

# Processing node Shortcut_to_EMPL_36HR_FLAG, type TARGET 
# COLUMN COUNT: 6

Shortcut_to_EMPL_36HR_FLAG = EXPTRANS.selectExpr(
...
Shortcut_to_EMPL_36HR_FLAG.write.mode("append").saveAsTable(f'{legacy}.EMPL_36HR_FLAG')
