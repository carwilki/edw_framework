...
SQ_Shortcut_to_WFA_ORG_FACT_PRE = spark.sql(
    f"""SELECT
...
SQ_Shortcut_to_WFA_TSCHD = spark.sql(
    f"""SELECT
...
SQ_Shortcut_to_WFA_TSCHD_DIFF_PRE = spark.sql(
    f"""SELECT
...
WHERE raw_wfa_tschd_diff_pre.org_skey = wfa_org_fact_pre.org_skey
    ...
    AND raw_wfa_tschd_diff_pre.tschd_dat = days.day_dt"""
).withColumn("sys_row_id", monotonically_increasing_id())
...
SQ_Shortcut_to_WFA_TSCHD_DIFF_PRE_temp = SQ_Shortcut_to_WFA_TSCHD_DIFF_PRE.toDF(
    *[
        "SQ_Shortcut_to_WFA_TSCHD_DIFF_PRE___" + col
        for col in SQ_Shortcut_to_WFA_TSCHD_DIFF_PRE.columns
    ]
)
SQ_Shortcut_to_WFA_ORG_FACT_PRE_temp = SQ_Shortcut_to_WFA_ORG_FACT_PRE.toDF(
    *[
        "SQ_Shortcut_to_WFA_ORG_FACT_PRE___" + col
        for col in SQ_Shortcut_to_WFA_ORG_FACT_PRE.columns
    ]
)

JNR_TO_PREV_ORG = SQ_Shortcut_to_WFA_ORG_FACT_PRE_temp.join(
    SQ_Shortcut_to_WFA_TSCHD_DIFF_PRE_temp,
    [
        SQ_Shortcut_to_WFA_ORG_FACT_PRE_temp.SQ_Shortcut_to_WFA_ORG_FACT_PRE___ORG_SKEY
        == SQ_Shortcut_to_WFA_TSCHD_DIFF_PRE_temp.SQ_Shortcut_to_WFA_TSCHD_DIFF_PRE___PRI_ORG_SKEY
    ],
    "right_outer",
).selectExpr(
...
# for each involved DataFrame, append the dataframe name to each column
SQ_Shortcut_to_WFA_TSCHD_temp = SQ_Shortcut_to_WFA_TSCHD.toDF(
    *["SQ_Shortcut_to_WFA_TSCHD___" + col for col in SQ_Shortcut_to_WFA_TSCHD.columns]
)
JNR_TO_PREV_ORG_temp = JNR_TO_PREV_ORG.toDF(
    *["JNR_TO_PREV_ORG___" + col for col in JNR_TO_PREV_ORG.columns]
)

JNR_TRANS = SQ_Shortcut_to_WFA_TSCHD_temp.join(
    JNR_TO_PREV_ORG_temp,
    [
        SQ_Shortcut_to_WFA_TSCHD_temp.SQ_Shortcut_to_WFA_TSCHD___DAY_DT
        == JNR_TO_PREV_ORG_temp.JNR_TO_PREV_ORG___TSCHD_DAT,
        SQ_Shortcut_to_WFA_TSCHD_temp.SQ_Shortcut_to_WFA_TSCHD___TSCHD_ID
        == JNR_TO_PREV_ORG_temp.JNR_TO_PREV_ORG___TSCHD_ID,
    ],
    "right_outer",
).selectExpr(
...
try:
    primary_key = (
        """source.DAY_DT = target.DAY_DT AND source.TSCHD_ID = target.TSCHD_ID"""
    )
