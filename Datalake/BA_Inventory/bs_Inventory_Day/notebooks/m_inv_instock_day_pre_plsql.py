...
# # Processing node SQ_Shortcut_To_INVENTORY_PRE_GENERIC, type SOURCE 
# # COLUMN COUNT: 2

# _sql = f"""
# INSERT OVERWRITE {raw}.INV_INSTOCK_PRE
# SELECT (to_date('2023-10-21','yyyy-MM-dd') - INTERVAL 1 DAY) AS DAY_DT,
#     POGLST.PRODUCT_ID,
#     POGLST.LOCATION_ID,
#     0 AS AVG_QTY,
...
#     COALESCE(MAX(SKU_CAPACITY_QTY), 0) SKU_CAPACITY_QTY,
#     current_date AS LOAD_DT,
#     COALESCE(MAX(SKU_FACINGS_QTY), 0) SKU_FACINGS_QTY
# FROM (
#     SELECT COALESCE(SKUSUB.PRODUCT_ID, POGLST.PRODUCT_ID) AS PRODUCT_ID,
#         POGLST.LOCATION_ID AS LOCATION_ID,
#         0 AS AVG_QTY,
#         CASE WHEN INVDAY.PRODUCT_ID IS NOT NULL THEN 0 ELSE 1 END AS OUT_OF_STOCK_IND,
#         POGLST.POG_LISTED_FLAG,
#         CASE WHEN SKUSUB.PRODUCT_ID IS NULL OR SKUSUB.PRODUCT_ID = SKUSUB.JOIN_PRODUCT_ID THEN POGLST.SAP_LISTED_FLAG ELSE 0 END AS SAP_LISTED_IND,
#         CASE WHEN SKUSUB.PAR_FLAG IS NULL OR SKUSUB.PRODUCT_ID = SKUSUB.JOIN_PRODUCT_ID THEN POGLST.INLINE_FLAG ELSE 0 END AS INLINE_IND,
#         CASE WHEN SKUSUB.PAR_FLAG IS NULL OR SKUSUB.PRODUCT_ID = SKUSUB.JOIN_PRODUCT_ID THEN POGLST.PLANNER_FLAG ELSE 0 END AS PLANNER_IND,
...
#                 P_I.LOCATION_ID,
#                 CASE WHEN MIN(P_I.POG_TYPE_CD) = 'F' THEN 1
#                      WHEN MIN(P_I.POG_TYPE_CD) = 'I' AND MAX(P_I.POG_D7_CD) = 'I' THEN 1
#                      ELSE 0
#                 END AS INLINE_FLAG,
#                 CASE WHEN MAX(P_I.POG_TYPE_CD) = 'P' THEN 1 ELSE 0 END AS PLANNER_FLAG
#             FROM (
...
#                     JOIN {legacy}.planogram_pro pp ON pzp.POG_DBKEY = pp.POG_DBKEY
#                     JOIN {legacy}.pog_sku_pro pskp ON pp.POG_ID = pskp.POG_ID
#                     JOIN {legacy}.pog_store_pro pstp ON pp.POG_ID = pstp.POG_ID
#                     JOIN {legacy}.sku_profile sku ON pzp.product_id = sku.product_id
#                     JOIN {legacy}.site_profile site ON pzp.location_id = site.location_id
#                 WHERE to_date('2023-10-21','yyyy-MM-dd') - INTERVAL 1 DAY BETWEEN pzp.LISTING_START_DT AND pp.EFF_END_DT
...
#                     tmp.LOCATION_ID,
#                     CASE WHEN tmp.POG_TYPE_CD <> 'I' THEN tmp.POG_TYPE_CD
#                          WHEN tmp.POG_TYPE_CD = 'I' AND d.negate_flag = 'X' THEN 'J'
#                          WHEN tmp.POG_TYPE_CD = 'I' AND d.negate_flag IS NULL THEN 'J'
#                          WHEN tmp.POG_TYPE_CD = 'I' AND d.negate_flag <> 'X'
#                               AND d.negate_flag IS NOT NULL
#                               AND d.listing_eff_dt + INTERVAL 28 DAY <= (to_date('2023-10-21','yyyy-MM-dd') - INTERVAL 1 DAY)
#                               AND p_status = 14 THEN tmp.POG_TYPE_CD
#                          WHEN tmp.POG_TYPE_CD = 'I' AND d.negate_flag <> 'X'
#                               AND d.negate_flag IS NOT NULL
#                               AND d.listing_eff_dt + INTERVAL 28 DAY <= (to_date('2023-10-21','yyyy-MM-dd') - INTERVAL 1 DAY)
#                               AND p_status = 30 THEN 'F'
#                          WHEN tmp.POG_TYPE_CD = 'I' AND d.negate_flag <> 'X'
#                               AND d.negate_flag IS NOT NULL
#                               AND d.listing_eff_dt + INTERVAL 28 DAY > (to_date('2023-10-21','yyyy-MM-dd') - INTERVAL 1 DAY)
#                               THEN 'J'
#                         END AS POG_TYPE_CD,
#                     CASE WHEN tmp.POG_TYPE_CD = 'I'
#                               AND d.negate_flag IS NOT NULL
#                               AND d.negate_flag <> 'X'
#                               AND d.listing_eff_dt + INTERVAL 28 DAY > (to_date('2023-10-21','yyyy-MM-dd') - INTERVAL 1 DAY)
#                               AND p_status = 30 THEN 'J'
#                          WHEN tmp.POG_TYPE_CD = 'I'
#                               AND d.negate_flag = 'X' THEN 'J'
#                          ELSE 'I'
#                     END AS POG_D7_CD
#                 FROM (
...
#                         si.STORE_NBR,
#                         pp.POG_TYPE_CD,
#                         CASE WHEN UPPER(SUBSTR(pp.pog_nbr, 1, 2)) IN (SELECT UPPER(status_id) FROM {legacy}.pog_status) THEN 30
#                              ELSE 14
#                         END AS p_status
#                     FROM {legacy}.POG_SKU_PRO PSK
#                         JOIN {legacy}.POG_STORE_PRO PST ON PSK.POG_ID = PST.POG_ID
#                         JOIN {legacy}.PLANOGRAM_PRO PP ON PSK.POG_ID = PP.POG_ID
#                         JOIN {legacy}.sku_profile s ON PSK.PRODUCT_ID = s.PRODUCT_ID
#                         JOIN {legacy}.site_profile si ON PST.LOCATION_ID = si.LOCATION_ID
#                     WHERE PSK.POG_STATUS <> 'D'
#                         AND PST.POG_STATUS <> 'D'
#                         AND PP.POG_STATUS <> 'D'
#                         AND PP.POG_TYPE_CD = 'I'
#                     ) tmp
...
#         CASE WHEN SUBS_IND = 0 THEN 1 ELSE 0 END AS PAR_FLAG,
#         1 AS SUBS_FLAG
...
#             JOIN {legacy}.SKU_PROFILE SKU ON IP.SKU_NBR = SKU.SKU_NBR
#             JOIN {legacy}.SITE_PROFILE SP ON IP.STORE_NBR = SP.STORE_NBR
#         ) INV
...
# AND DATE_TRUNC('DAY', (to_date('2023-10-21','yyyy-MM-dd') - INTERVAL 1 DAY)) = INVDAY.DAY_DT
# AND INVDAY.ON_HAND_QTY - COALESCE(INVDAY.QUANTITY, 0) > 0
# ) POGLST
# LEFT OUTER JOIN (
# SELECT PRODUCT_ID,
#     LOCATION_ID,
...
# GROUP BY POGLST.PRODUCT_ID, POGLST.LOCATION_ID
