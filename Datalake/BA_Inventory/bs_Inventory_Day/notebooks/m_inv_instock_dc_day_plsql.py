...
#Code converted on 2023-09-15 14:55:24
import os
import argparse
...
#           source_vendor_id, status_id, site_sales_flag AS store_open_ind,
#           out_of_stock_ind, pog_listed_ind, sap_listed_ind, inline_ind,
#           planner_ind, subs_ind, map_amt,
#           CASE
...
#           CASE
#              WHEN retail_price_amt IS NOT NULL
#              AND TRIM (country_cd) = 'US'
#                 THEN retail_price_amt
#              WHEN retail_price_amt IS NOT NULL AND TRIM (country_cd) = 'CA'
...
#              WHEN TRIM (country_cd) = 'CA'
#                 THEN ROUND(NVL(nat_price_ca_amt, nat_price_us_amt) / c.exchange_rate_pcnt, 2)
#              ELSE 0
...
#         WHERE day_dt = (to_date('2023-10-28','yyyy-MM-dd') - INTERVAL 1 DAY)) c,
...
#               NVL(sc.from_location_id, 90000) 
#               from_location_id,
#               NVL(sc.source_vendor_id, 1802) source_vendor_id,
#               sp.site_sales_flag, sp.company_id, 
...
#               NVL(planner_ind, 0) planner_ind, 
#               NVL(subs_ind, 0) subs_ind,
#               NVL(map_amt, 0) map_amt, 
...
#               pd.loc_petperks_price_amt, country_cd, sku_facings_qty
#          FROM {raw}.inv_instock_pre iip 
#              LEFT OUTER JOIN {raw}.inventory_day_pre idp ON iip.day_dt = idp.day_dt
...
#                              FROM {legacy}.sku_site_profile ssp) pd 
#               ON iip.product_id = pd.product_id
...
#           (SELECT location_id, TRIM (country_cd) AS country_cd, site_sales_flag, company_id
#              FROM {legacy}.site_profile
#             WHERE store_type_id != '120') sp
...
_sql = f"""INSERT INTO {legacy}.inv_instock_price_day
   SELECT day_dt, iid.product_id, iid.location_id, from_location_id,
     ...
     FROM (SELECT exchange_rate_pcnt
           ...
           (SELECT iip.day_dt, iip.product_id, iip.location_id,
sku.nat_price_us_amt, sku.nat_price_ca_amt, sku.nat_price_pr_amt,   sku.status_id,
                  NVL (sc.from_location_id, 90000) from_location_id,
                  NVL (sc.source_vendor_id, 1802) source_vendor_id,
                  ...
                  NVL (planner_ind, 0) planner_ind, NVL (subs_ind,
                                                         0) subs_ind,
                  NVL (map_amt, 0) map_amt, NVL (on_hand_qty, 0) on_hand_qty,
                  NVL (committed_qty, 0) committed_qty,
                  NVL (xfer_in_trans_qty, 0) xfer_in_trans_qty,
                  NVL (on_order_qty, 0) on_order_qty,
