...
# # Processing node SQ_Shortcut_to_ZTB_INV_TRANS, type APPLICATION_SOURCE_QUALIFIER 
# # COLUMN COUNT: 4

# dbutils.widgets.text(name='source_bucket', defaultValue='gs://petm-bdpl-qa-raw-p1-gcs-gbl/sap/inventory/ztb_inv_trans/')
...
# SQ_Shortcut_to_ZTB_INV_TRANS = spark.read.csv(source_file, sep='|', header=True).withColumn("sys_row_id", monotonically_increasing_id())
...
SQ_Shortcut_to_ZTB_INV_TRANS_temp = SQ_Shortcut_to_ZTB_INV_TRANS.toDF(*["SQ_Shortcut_to_ZTB_INV_TRANS___" + col for col in SQ_Shortcut_to_ZTB_INV_TRANS.columns])
...
JNR_SITE_PROFILE = SQ_Shortcut_To_SITE_PROFILE.join(EXP_INT,[SQ_Shortcut_To_SITE_PROFILE.STORE_NBR == EXP_INT.SITE],'right_outer')
...
JNR_SKU_PROFILE = SQ_Shortcut_To_SKU_PROFILE.join(JNR_SITE_PROFILE,[SQ_Shortcut_To_SKU_PROFILE.SKU_NBR == JNR_SITE_PROFILE.ARTICLE],'right_outer')
...
Shortcut_to_INV_UNAVAILABLE_HIST.write.mode("append").saveAsTable(f'{legacy}.INV_UNAVAILABLE_HIST')
logPrevRunDt("INV_UNAVAILABLE_HIST", "INV_UNAVAILABLE_HIST", "Completed", "N/A", f"{raw}.log_run_details")
