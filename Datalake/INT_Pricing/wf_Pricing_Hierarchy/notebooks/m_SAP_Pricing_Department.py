...
#Code converted on 2023-09-19 11:15:44
import os
import argparse
...
# Processing node FIL_LVL3, type FILTER 
# COLUMN COUNT: 5
...
FIL_LVL3 = SQ_Shortcut_to_SAP_PRICING_HIERARCHY_PRE_temp.selectExpr(
	...
	"SQ_Shortcut_to_SAP_PRICING_HIERARCHY_PRE___NODE_DESC as NODE_DESC").filter("HIER_LEVEL = 3 AND END_DT = date'9999-12-31'").withColumn("sys_row_id", monotonically_increasing_id())
...
Union_INS_DUMMY_REC = FIL_LVL3.select(
	col('HIER_NODE'),
	col('PARENT_NODE'),
...
# Processing node Shortcut_to_SAP_PRICING_DEPT, type TARGET 
# COLUMN COUNT: 4


Shortcut_to_SAP_PRICING_DEPT = EXP_LVL3.selectExpr(
	"CAST(o_HIER_NODE AS INT) as SAP_PRICING_DEPT_ID",
	"CAST(o_PARENT_NODE AS INT) as SAP_PRICING_DIVISION_ID",
	"CAST(NODE_DESC AS STRING) as SAP_PRICING_DEPT_DESC",
	"CAST(o_LOAD_TSTMP AS TIMESTAMP) as LOAD_TSTMP"
)
Shortcut_to_SAP_PRICING_DEPT.write.mode("overwrite").saveAsTable(f'{legacy}.SAP_PRICING_DEPT')
