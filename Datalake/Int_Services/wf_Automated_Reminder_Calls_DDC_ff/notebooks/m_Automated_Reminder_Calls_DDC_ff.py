...
QueryCond = getParameterValue(
    ...
    "INT_Services.WF:wf_Automated_Reminder_Calls_DDC_ff",
    "QueryCond",
)
...
DDCSAPClassIds = getParameterValue(
    raw,
    "Services_Daily_Parameter.prm",
    "INT_Services.WF:wf_Automated_Reminder_Calls_DDC_ff",
    "DDCSAPClassIds",
...
DDCSAPCategoryIds = getParameterValue(
    raw,
    "Services_Daily_Parameter.prm",
    "INT_Services.WF:wf_Automated_Reminder_Calls_DDC_ff",
    "DDCSAPCategoryIds",
...
LKP_FIRSTTIME_DDC_CUSTOMER_SRC = spark.sql(
    f"""
...
# Conforming fields names to the component layout
LKP_FIRSTTIME_DDC_CUSTOMER_SRC = LKP_FIRSTTIME_DDC_CUSTOMER_SRC.withColumnRenamed(
    LKP_FIRSTTIME_DDC_CUSTOMER_SRC.columns[0], "TP_CUSTOMER_NBR"
...
FROM {cust_sensitive}.refine_tp_invoice_rpt a JOIN {legacy}.SITE_PROFILE_RPT s ON a.location_id=s.location_id
 ...
 and a.APPT_START_TSTMP <= current_date + interval 6 day
...
FIL_ReminderCalls = (
    ...
    .filter(
        expr(
            "(Holiday_Store_Today IS NULL AND date_trunc('day', APPT_START_TSTMP) = date_trunc('day', ApptDate)) OR ( Holiday_Store_NextDay IS NOT NULL AND date_trunc('day', APPT_START_TSTMP) = date_trunc('day', Holiday_ApptDate))"
        )
    )
    .withColumn("sys_row_id", monotonically_increasing_id())
)
...
EXP_Agg_Reminders = EXP_Agg_Reminders.join(
    LKP_FIRSTTIME_DDC_CUSTOMER_SRC,
    (
        LKP_FIRSTTIME_DDC_CUSTOMER_SRC.TP_CUSTOMER_NBR
        == EXP_Agg_Reminders.TP_CUSTOMER_NBR
    ),
    "left",
).select(
    EXP_Agg_Reminders["*"],
    LKP_FIRSTTIME_DDC_CUSTOMER_SRC["APPT_START_TSTMP"].alias("ULKP_RETURN_9"),
)

EXP_Agg_Reminders = EXP_Agg_Reminders.withColumn("Min_ApptTime", col("ULKP_RETURN_9"))

EXP_Agg_Reminders_temp = EXP_Agg_Reminders.toDF(
    *["EXP_Agg_Reminders___" + col for col in EXP_Agg_Reminders.columns]
)

FIL_FirstTime_DDCCustomer = (
    EXP_Agg_Reminders_temp.selectExpr(
        ...
        "EXP_Agg_Reminders___APPT_START_TSTMP as APPT_START_TSTMP",
    )
    .filter(
        expr("(DATE_TRUNC('day', Min_ApptTime) = DATE_TRUNC('day', APPT_START_TSTMP))")
    )
    .withColumn("sys_row_id", monotonically_increasing_id())
)
