...
QueryCond=getParameterValue(raw,"Services_Daily_Parameter.prm","INT_Services.WF:wf_Automated_Reminder_Calls_PetsHotel_ff","QueryCond")

#LastRunDate=args.LastRunDate
LastRunDate=getParameterValue(raw,"Services_Daily_Parameter.prm","INT_Services.WF:wf_Automated_Reminder_Calls_PetsHotel_ff","LastRunDate")

# LastRunDate="2023-07-10"

#RunDate=args.RunDate
#RunDate="2023-07-17"
#RunDate = str(date.today() - timedelta(days=1))
RunDate="2023-07-24"
...
# PRStoreList = "('x')"
PRStoreList=getParameterValue(raw,"Services_Daily_Parameter.prm","INT_Services.WF:wf_Automated_Reminder_Calls_PetsHotel_ff","PRStoreList")

#PetsHotelSAPClassIds=args.PetsHotelSAPClassIds
# PetsHotelSAPClassIds= "(sap_class_id)"
PetsHotelSAPClassIds=getParameterValue(raw,"Services_Daily_Parameter.prm","INT_Services.WF:wf_Automated_Reminder_Calls_PetsHotel_ff","PetsHotelSAPClassIds")

#NormalAppt=args.NormalAppt
# NormalAppt=3
NormalAppt=getParameterValue(raw,"Services_Daily_Parameter.prm","INT_Services.WF:wf_Automated_Reminder_Calls_PetsHotel_ff","NormalAppt")

#HolidayAppt=args.HolidayAppt
# HolidayAppt=2
HolidayAppt=getParameterValue(raw,"Services_Daily_Parameter.prm","INT_Services.WF:wf_Automated_Reminder_Calls_PetsHotel_ff","HolidayAppt")

#PeakPeriodAppt=args.PeakPeriodAppt
# PeakPeriodAppt = 4
PeakPeriodAppt=getParameterValue(raw,"Services_Daily_Parameter.prm","INT_Services.WF:wf_Automated_Reminder_Calls_PetsHotel_ff","PeakPeriodAppt")

#PeakPeriod_HolidayApptDate=args.PeakPeriod_HolidayApptDate
# PeakPeriod_HolidayApptDate=3
PeakPeriod_HolidayApptDate=getParameterValue(raw,"Services_Daily_Parameter.prm","INT_Services.WF:wf_Automated_Reminder_Calls_PetsHotel_ff","PeakPeriod_HolidayApptDate")

# i_TP_CUSTOMER_NBR = None    # ??? Should we get it as a parameter ???
# i_TP_PHONE_TYPE_ID = None   # ??? Should we get it as a parameter ???
# i_Service_Name = None       # ??? Should we get it as a parameter ???
# i_Country_Cd = None         # ??? Should we get it as a parameter ???
# i_Date = None               # ??? Should we get it as a parameter ???
# i_RuleName = None           # ??? Should we get it as a parameter ???
# i_StoreNbr = 123            # ??? Should we get it as a parameter ???
...
LKP_RULES_SRC = spark.sql(f"""SELECT
SERVICE_NAME,
RULE_NAME,
PEAK_START_DATE,
PEAK_END_DATE,
...
# Conforming fields names to the component layout
LKP_RULES_SRC = (LKP_RULES_SRC
	.withColumnRenamed(LKP_RULES_SRC.columns[0],'SERVICE_NAME')
...
FROM {cust_sensitive}.refine_tp_invoice_rpt a JOIN {legacy}.SITE_PROFILE_RPT s ON a.location_id=s.location_id
 ...
 JOIN {legacy}.SKU_PROFILE_RPT p ON p.product_id = b.product_id
  AND p.sap_class_id in {PetsHotelSAPClassIds}""").withColumn("sys_row_id", monotonically_increasing_id())
...
SRT_Reservations = TP_INVOICE_RPT.select('*').sort(col('TP_CUSTOMER_NBR').asc(), col('TP_INVOICE_NBR').asc(), col('APPT_START_TSTMP').asc(), col('STORE_NBR').asc(), col('TP_PET_NBR').asc())

SRT_Reservations_1 =  TP_INVOICE_RPT.select("TP_CUSTOMER_NBR","APPT_START_TSTMP","APPT_END_TSTMP","TP_INVOICE_NBR","STORE_NBR").withColumn(
  "rownum", row_number().over(Window.partitionBy(col("TP_CUSTOMER_NBR"), to_date(col("APPT_START_TSTMP")), col("STORE_NBR")).orderBy(desc(col("APPT_START_TSTMP")),desc(col("TP_INVOICE_NBR"))))).filter(col("rownum")==1)
...
v_PeakPeriod_HolidayApptDate = v_NextRunDate + timedelta(days=int(PeakPeriod_HolidayApptDate))
...
EXP_AutomatedCalls = exp.join(lkp,
...
EXP_AutomatedCalls = EXP_AutomatedCalls.withColumn('v_PeakPeriod_Appt',col('ULKP_RETURN_3'))

# Adding deferred logic for dataframe EXP_AutomatedCalls, column PeakPeriod_CheckDate
EXP_AutomatedCalls = EXP_AutomatedCalls.withColumn('PeakPeriod_CheckDate', expr("v_PeakPeriod_Appt as PeakPeriod_CheckDate"))
...
FIL_ReminderCalls = EXP_ReservationDates_temp.selectExpr(
	...
	"EXP_ReservationDates___PeakPeriod_HolidayApptDate as PeakPeriod_HolidayApptDate").filter(expr("(Holiday_Store_Today IS NULL AND DATE_TRUNC('day', APPT_START_TSTMP) = DATE_TRUNC('day', ApptDate)) OR " +"(NOT Holiday_Store_NextDay IS NULL AND DATE_TRUNC('day', APPT_START_TSTMP) = DATE_TRUNC('day', Holiday_ApptDate)) OR " +
                             ...
                             "(NOT Holiday_Store_NextDay IS NULL AND NOT PeakPeriod_CheckDate IS NULL AND DATE_TRUNC('day', APPT_START_TSTMP) = DATE_TRUNC('day', PeakPeriod_HolidayApptDate))")).withColumn("sys_row_id", monotonically_increasing_id())
...
FIL_ReminderCalls_a = FIL_ReminderCalls_a.join(SRT_Reservations_1_a,[col('b.TP_CUSTOMER_NBR') == col('a.TP_CUSTOMER_NBR'), col('b.APPT_START_TSTMP') == col('a.APPT_START_TSTMP'), col('b.STORE_NBR') == col('a.STORE_NBR')]).selectExpr(
...
AGG_ReminderCalls = (  FIL_ReminderCalls_a.selectExpr(
