...
source_file = get_file_for_processing(source_bucket)
...
SQ_Shortcut_to_STORE_DATA_FLAT1 = spark.read.csv(
    source_file, sep=";", header="false", inferSchema=True
).withColumn("sys_row_id", monotonically_increasing_id())
# Conforming fields names to the component layout
SQ_Shortcut_to_STORE_DATA_FLAT1 = (
    SQ_Shortcut_to_STORE_DATA_FLAT1.withColumnRenamed(
        SQ_Shortcut_to_STORE_DATA_FLAT1.columns[0], "SITE_NBR"
    )
    ...
    .withColumnRenamed(
        SQ_Shortcut_to_STORE_DATA_FLAT1.columns[4], "TAX_JURISDICTION_CD"
    )
    .withColumnRenamed(
        SQ_Shortcut_to_STORE_DATA_FLAT1.columns[5], "EARLIEST_VALUE_POST_DT"
    )
    .withColumnRenamed(
        SQ_Shortcut_to_STORE_DATA_FLAT1.columns[6], "TOT_VALUE_WARNING_LMT"
    )
    .withColumnRenamed(
        SQ_Shortcut_to_STORE_DATA_FLAT1.columns[7], "TOT_VALUE_ERROR_LMT"
    )
    .withColumnRenamed(
        SQ_Shortcut_to_STORE_DATA_FLAT1.columns[8], "TRANS_CNT_LOWER_LMT"
    )
    .withColumnRenamed(
        SQ_Shortcut_to_STORE_DATA_FLAT1.columns[9], "TRANS_CNT_WARNING_LMT"
    )
    .withColumnRenamed(
        SQ_Shortcut_to_STORE_DATA_FLAT1.columns[10], "TRANS_CNT_ERROR_LMT"
    )
    .withColumnRenamed(
        SQ_Shortcut_to_STORE_DATA_FLAT1.columns[11], "TRANS_VALUE_ERROR_LMT"
    )
    .withColumnRenamed(
        SQ_Shortcut_to_STORE_DATA_FLAT1.columns[12], "TRANS_SEQ_GAP_CNT_ERROR_LMT"
    )
    .withColumnRenamed(
        SQ_Shortcut_to_STORE_DATA_FLAT1.columns[13], "OUT_OF_BALANCE_CNT"
    )
    .withColumnRenamed(
        SQ_Shortcut_to_STORE_DATA_FLAT1.columns[14], "OUT_OF_BALANCE_VALUE"
    )
    .withColumnRenamed(
        SQ_Shortcut_to_STORE_DATA_FLAT1.columns[15], "BANK_DEPOSIT_VAR_PCT"
    )
...
SQ_Shortcut_to_STORE_DATA_FLAT1_temp = SQ_Shortcut_to_STORE_DATA_FLAT1.toDF(
    *[
        "SQ_Shortcut_to_STORE_DATA_FLAT1___" + col
        for col in SQ_Shortcut_to_STORE_DATA_FLAT1.columns
    ]
)
...
EXPTRANS = SQ_Shortcut_to_STORE_DATA_FLAT1_temp.selectExpr(
    ...
    "SQ_Shortcut_to_STORE_DATA_FLAT1___UPDATE_USER as UPDATE_USER",
).selectExpr(
...
try:
    ...
    logger.info(f"Merge with {refined_perf_table} completed")
    logPrevRunDt(
        "STORE_DATA", "STORE_DATA", "Completed", "N/A", f"{raw}.log_run_details"
    )
except Exception as e:
    logPrevRunDt("STORE_DATA", "STORE_DATA", "Failed", str(e), f"{raw}.log_run_details")
    raise e
