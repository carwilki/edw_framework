  ...
  FROM {legacy}.USER_RANKING_HIERARCHY U,
       ...
       (SELECT LOCATION_ID,
          ...
          FROM (SELECT T1.LOCATION_ID,
                       ...
                       ROUND((T1.NSAMT/T2.TOT_NSAMT) * 100,1) AS PCT_NSAMT,

                       SUM(ROUND((T1.NSAMT/T2.TOT_NSAMT) * 100,1)) OVER (PARTITION BY T1.CONSUM_ID ORDER BY T1.NSAMT DESC

                       ROWS BETWEEN 5000 PRECEDING AND CURRENT ROW ) AS CUME_SUM
                  ...
                  FROM (SELECT W.LOCATION_ID,
                        ...
                        (SELECT D.CONSUM_ID,
                                ...
                                SUM(SALES_AMT - RETURN_AMT - DISCOUNT_AMT + DISCOUNT_RETURN_AMT

                                    - POS_COUPON_AMT - SPECIAL_SALES_AMT + SPECIAL_RETURN_AMT

                                    + SPECIAL_SRVC_AMT) AS TOT_NSAMT
                          ...
                          GROUP BY D.CONSUM_ID) T2

                   WHERE T1.CONSUM_ID = T2.CONSUM_ID) X) TT

  WHERE U.HIERARCHY_NAME    = 'Store Decile'

    AND U.HIERARCHY_SUB_LVL = TT.CONSUM_DESC

    AND TT.CUME_SUM BETWEEN RANK_MIN_VALUE AND RANK_MAX_VALUE""").withColumn("sys_row_id", monotonically_increasing_id())
...
try:
	...
	chk.check_for_duplicate_primary_keys(Shortcut_to_STORE_RANK,["LOCATION_ID",
                    ...
                    "HIERARCHY_NAME",
                    "RANK_NAME"])
	Shortcut_to_STORE_RANK.write.mode("append").saveAsTable(f'{legacy}.STORE_RANK')
