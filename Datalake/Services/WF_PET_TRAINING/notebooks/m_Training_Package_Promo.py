#Code converted on 2023-08-09 16:37:38
import os
import argparse
...
DELETED_FLAG,
LOAD_TSTMP
FROM {legacy}.TRAINING_PACKAGE_PROMO""").withColumn("sys_row_id", monotonically_increasing_id())
...
JNR_TRAINING_PACKAGE_PROMO = SQ_Shortcut_to_TRAINING_PACKAGE_PROMO_temp.join(SQ_Shortcut_to_TRAINING_PACKAGE_PROMOTION_PRE_temp,[SQ_Shortcut_to_TRAINING_PACKAGE_PROMO_temp.SQ_Shortcut_to_TRAINING_PACKAGE_PROMO___TRAINING_PACKAGE_PROMO_ID == SQ_Shortcut_to_TRAINING_PACKAGE_PROMOTION_PRE_temp.SQ_Shortcut_to_TRAINING_PACKAGE_PROMOTION_PRE___PACKAGE_PROMOTION_ID],'right_outer').selectExpr( \
	...
	"SQ_Shortcut_to_TRAINING_PACKAGE_PROMO___DELETED_FLAG as lkp_DELETED_FLAG")
...
FIL_UNCHANGED_RECORDS = JNR_TRAINING_PACKAGE_PROMO_temp.selectExpr( \
	...
	"JNR_TRAINING_PACKAGE_PROMO___lkp_LOAD_TSTMP as lkp_LOAD_TSTMP", \
	"JNR_TRAINING_PACKAGE_PROMO___lkp_DELETED_FLAG as lkp_DELETED_FLAG").filter("lkp_TRAINING_PACKAGE_PROMO_ID IS NULL OR ( lkp_TRAINING_PACKAGE_PROMO_ID IS NOT NULL AND ( IF (LTRIM ( RTRIM ( NAME ) ) IS NULL, ' ', LTRIM ( RTRIM ( NAME ) )) != IF (LTRIM ( RTRIM ( lkp_TRAINING_PACKAGE_PROMO_NAME ) ) IS NULL, ' ', LTRIM ( RTRIM ( lkp_TRAINING_PACKAGE_PROMO_NAME ) )) OR IF (LTRIM ( RTRIM ( DESCRIPTION ) ) IS NULL, ' ', LTRIM ( RTRIM ( DESCRIPTION ) )) != IF (LTRIM ( RTRIM ( lkp_TRAINING_PACKAGE_PROMO_DESC ) ) IS NULL, ' ', LTRIM ( RTRIM ( lkp_TRAINING_PACKAGE_PROMO_DESC ) )) OR IF (DISCOUNT_AMOUNT IS NULL, cast(99 as int), DISCOUNT_AMOUNT) != IF (lkp_DISC_AMT IS NULL, cast(99 as int), lkp_DISC_AMT) OR IF (START_DATE IS NULL, To_DATE ( '12-31-9999' , 'MM-DD-YYYY' ), START_DATE) != IF (lkp_START_DT IS NULL, To_DATE ( '12-31-9999' , 'MM-DD-YYYY' ), lkp_START_DT) OR IF (END_DATE IS NULL, To_DATE ( '12-31-9999' , 'MM-DD-YYYY' ), END_DATE) != IF (lkp_END_DT IS NULL, To_DATE ( '12-31-9999' , 'MM-DD-YYYY' ), lkp_END_DT) OR IF (PACKAGE_ID IS NULL, cast(99 as int), PACKAGE_ID) != IF (PACKAGE_ID IS NULL, cast(99 as int), PACKAGE_ID) OR IF (LTRIM ( RTRIM ( COUNTRY_ABBREVIATION ) ) IS NULL, ' ', LTRIM ( RTRIM ( COUNTRY_ABBREVIATION ) )) != IF (LTRIM ( RTRIM ( lkp_COUNTRY_CD ) ) IS NULL, ' ', LTRIM ( RTRIM ( lkp_COUNTRY_CD ) )) ) )").withColumn("sys_row_id", monotonically_increasing_id())
...
EXP_UPDATE_VALIDATOR = FIL_UNCHANGED_RECORDS_temp.selectExpr( \
	...
	"IF (FIL_UNCHANGED_RECORDS___lkp_TRAINING_PACKAGE_PROMO_ID IS NULL, 1, 2) as UPDATE_FLAG", \
	"0 as DELETE_FLAG" \
)
...
# COMMAND ----------
# Processing node Shortcut_to_TRAINING_PACKAGE_PROMO1, type TARGET 
# COLUMN COUNT: 11


Shortcut_to_TRAINING_PACKAGE_PROMO1 = UPD_INSERT_UPDATE.selectExpr( \
	"CAST(PACKAGE_PROMOTION_ID AS BIGINT) as TRAINING_PACKAGE_PROMO_ID", \
	"CAST(NAME AS STRING) as TRAINING_PACKAGE_PROMO_NAME", \
	"CAST(DESCRIPTION AS STRING) as TRAINING_PACKAGE_PROMO_DESC", \
...
# Shortcut_to_TRAINING_PACKAGE_PROMO1.write.saveAsTable(f'{raw}.TRAINING_PACKAGE_PROMO', mode = 'overwrite')
spark.sql("""set spark.sql.legacy.timeParserPolicy = LEGACY""")

try:
  primary_key = """source.TRAINING_PACKAGE_PROMO_ID = target.TRAINING_PACKAGE_PROMO_ID"""
  refined_perf_table = f"{legacy}.TRAINING_PACKAGE_PROMO"
  executeMerge(Shortcut_to_TRAINING_PACKAGE_PROMO1, refined_perf_table, primary_key)
  logger.info(f"Merge with {refined_perf_table} completed]")
  logPrevRunDt("TRAINING_PACKAGE_PROMO", "TRAINING_PACKAGE_PROMO", "Completed", "N/A", f"{raw}.log_run_details")
except Exception as e:
  logPrevRunDt("TRAINING_PACKAGE_PROMO", "TRAINING_PACKAGE_PROMO","Failed",str(e), f"{raw}.log_run_details", )
  raise e
