#Code converted on 2023-08-09 16:37:37
import os
import argparse
...
DELETED_FLAG,
LOAD_TSTMP
FROM {legacy}.TRAINING_CLASS_PROMO
...
JNR_TRAINING_CLASS_PROMO = SQ_Shortcut_to_TRAINING_CLASS_PROMO_temp.join(SQ_Shortcut_to_TRAINING_CLASS_PROMOTION_PRE_temp,[SQ_Shortcut_to_TRAINING_CLASS_PROMO_temp.SQ_Shortcut_to_TRAINING_CLASS_PROMO___TRAINING_CLASS_PROMO_ID == SQ_Shortcut_to_TRAINING_CLASS_PROMOTION_PRE_temp.SQ_Shortcut_to_TRAINING_CLASS_PROMOTION_PRE___CLASS_PROMOTION_ID],'left_outer').selectExpr( \
...
FIL_UNCHANGED_RECORDS = JNR_TRAINING_CLASS_PROMO_temp.selectExpr( \
	...
	"JNR_TRAINING_CLASS_PROMO___lkp_LOAD_TSTMP as lkp_LOAD_TSTMP").filter("lkp_TRAINING_CLASS_PROMO_ID IS NOT NULL AND CLASS_PROMOTION_ID IS NULL").withColumn("sys_row_id", monotonically_increasing_id())
...
EXP_UPDATE_VALIDATOR = FIL_UNCHANGED_RECORDS_temp.selectExpr( \
	...
	"FIL_UNCHANGED_RECORDS___lkp_TRAINING_CLASS_PROMO_ID as lkp_TRAINING_CLASS_PROMO_ID", \
...
UPD_DELETE_FLAG = EXP_UPDATE_VALIDATOR_temp.selectExpr( \
	"EXP_UPDATE_VALIDATOR___lkp_TRAINING_CLASS_PROMO_ID as lkp_TRAINING_CLASS_PROMO_ID", \
...
# COMMAND ----------
# Processing node Shortcut_to_TRAINING_CLASS_PROMO11, type TARGET 
# COLUMN COUNT: 11

UPD_DELETE_FLAG.createOrReplaceTempView('UPD_DELETE_FLAG_TMP')
spark.sql(f"""
		MERGE INTO {legacy}.TRAINING_CLASS_PROMO target
        USING UPD_DELETE_FLAG_TMP source
        ON source.lkp_TRAINING_CLASS_PROMO_ID = target.TRAINING_CLASS_PROMO_ID
        WHEN MATCHED THEN UPDATE SET target.DELETED_FLAG = source.DELETE_FLAG , target.UPDATE_TSTMP = source.UPDATE_TSTMP
""")

# Shortcut_to_TRAINING_CLASS_PROMO11 = UPD_DELETE_FLAG.selectExpr( \
# 	"CAST(lkp_TRAINING_CLASS_PROMO_ID AS BIGINT) as TRAINING_CLASS_PROMO_ID", \
# 	"CAST(NULL AS STRING) as TRAINING_CLASS_PROMO_NAME", \
# 	"CAST(NULL AS STRING) as TRAINING_CLASS_PROMO_DESC", \
...
# )
# Shortcut_to_TRAINING_CLASS_PROMO11.write.saveAsTable(f'{raw}.TRAINING_CLASS_PROMO', mode = 'overwrite')