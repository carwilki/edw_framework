...
SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS = spark.sql(
    f"""SELECT
...
UPDATE_TSTMP,
LOAD_TSTMP
FROM {legacy}.E_RES_SELECTED_ADD_ONS"""
...
SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS_PRE = spark.sql(
    f"""SELECT
SELECTED_ADD_ON_ID,
PET_ID,
CATEGORY,
...
TOTAL_PRICE,
LOAD_TSTMP
FROM {raw}.E_RES_SELECTED_ADD_ONS_PRE"""
).withColumn("sys_row_id", monotonically_increasing_id())
...
SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS_temp = SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS.toDF(
    *[
        "SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS___" + col
        for col in SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS.columns
    ]
)
SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS_PRE_temp = (
    SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS_PRE.toDF(
        *[
            "SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS_PRE___" + col
            for col in SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS_PRE.columns
        ]
    )
)

JNR_E_RES_SELECTED_ADD_ONS = SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS_temp.join(
    SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS_PRE_temp,
    [
        SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS_temp.SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS___E_RES_SELECTED_ADD_ON_ID
        == SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS_PRE_temp.SQ_Shortcut_to_E_RES_SELECTED_ADD_ONS_PRE___SELECTED_ADD_ON_ID
    ],
    "right_outer",
).selectExpr(
...
# for each involved DataFrame, append the dataframe name to each column
JNR_E_RES_SELECTED_ADD_ONS_temp = JNR_E_RES_SELECTED_ADD_ONS.toDF(
    *[
        "JNR_E_RES_SELECTED_ADD_ONS___" + col
        for col in JNR_E_RES_SELECTED_ADD_ONS.columns
    ]
)

FIL_E_RES_SELECTED_ADD_ONS = (
    JNR_E_RES_SELECTED_ADD_ONS_temp.selectExpr(
        ...
        "JNR_E_RES_SELECTED_ADD_ONS___i_LOAD_TSTMP as i_LOAD_TSTMP",
    )
    .filter(
        "i_E_RES_SELECTED_ADD_ON_ID IS NULL OR ( i_E_RES_SELECTED_ADD_ON_ID IS NOT NULL AND ( IF(PET_ID IS NULL, - 99999, PET_ID) != IF(i_E_RES_PET_ID IS NULL, - 99999, i_E_RES_PET_ID) OR IF(CATEGORY IS NULL, '', CATEGORY) != IF(i_E_RES_ADD_ON_CATEGORY_NAME IS NULL, '', i_E_RES_ADD_ON_CATEGORY_NAME) OR IF(ADD_ON IS NULL, '', ADD_ON) != IF(i_E_RES_ADD_ON_DESC IS NULL, '', i_E_RES_ADD_ON_DESC) OR IF(FREQUENCY IS NULL, '', FREQUENCY) != IF(i_ADD_ON_FREQ IS NULL, '', i_ADD_ON_FREQ) OR IF(UNIT_PRICE IS NULL, - 99999, UNIT_PRICE) != IF(i_UNIT_PRICE_AMT IS NULL, - 99999, i_UNIT_PRICE_AMT) OR IF(TOTAL_PRICE IS NULL, - 99999, TOTAL_PRICE) != IF(i_TOTAL_PRICE_AMT IS NULL, - 99999, i_TOTAL_PRICE_AMT) ) )"
    )
    .withColumn("sys_row_id", monotonically_increasing_id())
)
...
# for each involved DataFrame, append the dataframe name to each column
FIL_E_RES_SELECTED_ADD_ONS_temp = FIL_E_RES_SELECTED_ADD_ONS.toDF(
    *[
        "FIL_E_RES_SELECTED_ADD_ONS___" + col
        for col in FIL_E_RES_SELECTED_ADD_ONS.columns
    ]
)
...
# for each involved DataFrame, append the dataframe name to each column
EXP_E_RES_SELECTED_ADD_ONS_temp = EXP_E_RES_SELECTED_ADD_ONS.toDF(
    *[
        "EXP_E_RES_SELECTED_ADD_ONS___" + col
        for col in EXP_E_RES_SELECTED_ADD_ONS.columns
    ]
)
