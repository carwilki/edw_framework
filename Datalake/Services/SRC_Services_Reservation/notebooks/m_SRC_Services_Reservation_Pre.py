...
RNK_SINGLE_RECORD_PER_CUSTOMER = FIL_NULL_EMAILS.withColumn('RANKINDEX', row_number().over(Window.partitionBy(col('TP_CUSTOMER_NBR'), col('CREATE_DT')).orderBy(col('TP_INVOICE_NBR').desc(), col('SKU_DESC').desc()))).filter('RANKINDEX <= 1')
