...
#Code converted on 2023-08-09 13:02:37
import os
import argparse
...
SELECT fiscal_mo 
      ...
      ,gl_category_cd 
      ,profit_ctr 
      ,NVL (location_id, 90000) location_id 
      ,curr_cd 
      ,loc_plan_amt 
      ,us_plan_amt 
      ,gl_forecast_amt_loc 
...
FROM (
    SELECT
        ...
        CASE
            WHEN gpfm.location_id IS NULL THEN 'I'
            WHEN gpfm.location_id IS NOT NULL
                 AND pre.loc_plan_amt = gpfm.gl_plan_amt_loc
                 AND pre.us_plan_amt = gpfm.gl_plan_amt_us THEN 'D'
            ELSE 'U'
        END AS row_action
...
SQ_Shortcut_to_GL_PLAN_FORECAST_MONTH = SQ_Shortcut_to_GL_PLAN_FORECAST_MONTH \
	...
	.withColumnRenamed(SQ_Shortcut_to_GL_PLAN_FORECAST_MONTH.columns[12],'ROW_ACTION')
...
UPD_ins_upd = SQ_Shortcut_to_GL_PLAN_FORECAST_MONTH_temp.selectExpr(
	...
	"SQ_Shortcut_to_GL_PLAN_FORECAST_MONTH___LOAD_DT as LOAD_DT1",
...
Shortcut_to_GL_PLAN_FORECAST_MONTH_ins_upd = UPD_ins_upd.selectExpr(
